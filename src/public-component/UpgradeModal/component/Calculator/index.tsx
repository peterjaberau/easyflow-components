...
import { calculatorStyle, calculatorTitleStyle, goodsStyle, orStyle } from './style';
...
export const Calculator: FC<CalculatorProps> = ({ changeNum, unitCollaByCycle }) => {
    ...
    <div css={calculatorStyle}>
      ...
      <div css={goodsStyle}>
        <span>{`${t('billing.payment_sidebar.colla.2', {
          storageNum: `${changeNum * unitCollaByCycle * UNIT_COLLA_CONVERSION_STORAGE}GB`,
        })}`}</span>
        <span css={orStyle}>{t('or')}</span>
        <span>
          {`${t('billing.payment_sidebar.colla.3', {
            trafficNum: `${changeNum * unitCollaByCycle * UNIT_COLLA_CONVERSION_TRAFFIC}GB`,
          })}`}
        </span>
        <span css={orStyle}>{t('or')}</span>
        <span>
          {`${t('billing.payment_sidebar.colla.4', {
            tokenNum: `${changeNum * unitCollaByCycle * UNIT_COLLA_CONVERSION_TOKEN}k`,
          })}`}
        </span>
      </div>
    </div>
