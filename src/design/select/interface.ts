...
import { TriggerTrigger } from '../trigger';
...
export type SelectValue = SelectOptionObject | string | SelectOptionObject[] | string[] | number | number[];

export interface SelectProps<T extends SelectValue = SelectValue>
  extends Omit<
      HTMLAttributes<HTMLDivElement>,
      'defaultValue' | 'onChange' | 'prefix' | 'onFocus' | 'onBlur' | 'onSelect' | 'children'
    >,
    BoxProps {
  ...
  showSearch?: boolean;
  value?: T;
  filterOption?: boolean | ((inputValue: string | number, option: SelectOptionObject) => boolean);
  defaultFilterOption?: boolean | ((inputValue: string | number, option: SelectOptionObject) => boolean);
