...
export const panelContainerStyle = (variant: TabVariant, tabPosition: TabPosition) => {
  let padding;
  switch (variant) {
    default:
    case 'line':
...
export const titleStyle = (
  ...
  if (tabsItemColorScheme) {
    itemCss = css`
      ...
      color: ${tabsItemColorScheme ?? getSpecialThemeColor(colorScheme ?? 'blue')};
      font-weight: 500;
      color: transparent;
    `;
  } else if (disabled) {
    ...
    if (tabsItemActiveColorScheme) {
      itemCss = css`
        ...
        -webkit-background-clip: text;
        -webkit-text-fill-color: transparent;
        color: ${tabsItemActiveColorScheme ?? getSpecialThemeColor(colorScheme ?? 'blue')};
        font-weight: 500;
        color: transparent;
      `;
    } else {
