...
import { Input, Loading, useMergeValue, useMessage } from '../../../../../design/react';
...
export const InviteByEmailMobile: FC<InviteByEmailProps> = (props) => {
  ...
  const handleInvite = useCallback(
    async (e: KeyboardEvent<HTMLInputElement>) => {
      ...
      if ((isBiggerThanTargetRole(USER_ROLE.EDITOR, inviteUserRole) && currentBalance === 0) || currentBalance < 0) {
        upgradeModal({
          modalType: 'upgrade',
      ...
      try {
        ...
        const currentIndex = finalInviteUserList.findIndex((item) => item.email === currentValue);
        const user = {
          email: currentValue,
  ...
  return (
    <div css={inviteByEmailContainerStyle(inviting)}>
      ...
      <div css={licenseContainerStyle}>
        ...
        <div css={applyLicenseNumberStyle(!!currentBalance && currentBalance > 0)}>{currentBalance ?? 0}</div>
      </div>
