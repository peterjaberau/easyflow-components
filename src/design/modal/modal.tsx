...
import { CloseIcon, ErrorCircleIcon, InfoCircleIcon, SuccessCircleIcon, WarningCircleIcon } from '../icon';
...
export const Modal = forwardRef<HTMLDivElement, ModalProps>((props, ref) => {
  ...
  const normalIcon: ReactNode = useMemo(() => {
    if (type && title) {
      switch (type) {
        case 'info':
          return <InfoCircleIcon fs="16px" mr="8px" c={getColor('blue', '03')} />;
        case 'error':
          return <ErrorCircleIcon fs="16px" mr="8px" c={getColor('red', '03')} />;
        case 'success':
          return <SuccessCircleIcon fs="16px" mr="8px" c={getColor('green', '03')} />;
        case 'warning':
          return <WarningCircleIcon fs="16px" mr="8px" c={getColor('red', '03')} />;
        default:
          return <></>;
      }
    } else {
              ...
              >
                      ...
                      <>
                        {closeElement ? (
                          ...
                          <div css={modalCloseIconStyle} onClick={onCancel}>
                            <CloseIcon size="14px" c={getColor('grayBlue', '02')} />
                          </div>
                        )}
                      </>
                    )}
                ...
                {focusLock ? (
                  <FocusLock disabled={!visible} autoFocus={autoFocus}>
                    {children && <div css={applyModalContent(withoutPadding)}>{children}</div>}
                  </FocusLock>
                ) : (
                  children && <div css={applyModalContent(withoutPadding)}>{children}</div>
                )}
