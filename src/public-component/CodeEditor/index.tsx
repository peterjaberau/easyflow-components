...
import { getStringSnippets, isDynamicStringSnippet } from '../dynamicStringUtils';
...
export const CodeEditor = forwardRef<HTMLDivElement, CodeEditorProps>((props) => {
  const {
    value = '',
    onChange = () => {},
    placeholder,
  ...
  const stringSnippets = useMemo(() => {
    ...
    dynamicStrings.forEach((stringSnippet) => {
      if (isDynamicStringSnippet(stringSnippet)) {
        ...
        if (!currentKey || completionOptions.find((item) => item.key.trim() === currentKey)) {
  ...
  return (
    <div css={[ILLACodeMirrorWrapperStyle, wrapperCss, illaCodeMirrorTooltipStyle]}>
      <ILLACodeMirrorCore
        className={className}
