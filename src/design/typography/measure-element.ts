...
function getContentHeight(contentRef: HTMLSpanElement): number {
  let height = 0;
  for (const c of contentRef.getClientRects()) {
    height += c.height;
  }
  return height;
}
...
export function measureElement(
  ...
  const computeElement = document.createElement(contentRef.tagName);
  ...
  const styleString = styleNames.map((name) => `${name}: ${originStyle.getPropertyValue(name)};`).join('');
