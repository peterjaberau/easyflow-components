...
import { InputTag, TagObject } from '../input-tag';
import { getLabelListFromValueList } from './util';
import { InputTagRefHandler } from '../input-tag';

export const MultipleCascader = forwardRef<HTMLDivElement, CascaderProps>((props, ref) => {
  const {
    size = 'medium',
    allowClear,
    placeholder,
  ...
  const [finalValue, setFinalValue] = useMergeValue<string[][]>([], {
    defaultValue: defaultValue !== undefined ? (defaultValue as string[][]) : undefined,
  ...
  return (
    ...
    >
      <InputTag
        ...
        labelInValue
        onRemove={(v) => {
          const index = finalValue.findIndex((item) => item.join(' / ') === (v as TagObject).value);
