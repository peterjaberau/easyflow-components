...
export function applyCard(bordered?: boolean, hoverable?: boolean): SerializedStyles {
  const border = bordered ? `border: solid 1px ${globalColor(`--${illaPrefix}-grayBlue-08`)}` : ``;

  const boxShadow = hoverable ? `box-shadow: 0 4px 10px 0 ${globalColor(`--${illaPrefix}-blackAlpha-07`)}` : '';
  return css`
    font-size: 14px;
...
export function applyCardMetaBody(size: CardSize, bordered?: boolean, cover?: ReactNode): SerializedStyles {
  const paddingSize = size === 'small' ? `8px` : `16px`;

  const border = bordered ? `solid 1px ${globalColor(`--${illaPrefix}-grayBlue-08`)}` : ``;
  return css`
    display: flex;
    flex-direction: column;
    box-sizing: border-box;
