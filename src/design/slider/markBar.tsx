...
import { mergeRefs } from '../system';
import React from 'react';

export const MarkBar = forwardRef<HTMLDivElement, SliderMarkBar>((props, ref) => {
  const {
    markBarRef,
    isRange,
    left,
  ...
  return (
    <motion.div
      ...
      dragElastic={0}
      dragConstraints={getMarkBound(rect.width / 2, value, location, partLength, max, step)}
      style={{ x: rightVal, zIndex: 2 }}
      dragMomentum={false}
      dragTransition={{
        timeConstant: 0,
        power: 0,
        modifyTarget: (target) => Math.round(target / partLength) * partLength - rect.width / 2,
      }}
    >
      <div css={applyBarContainer} onMouseEnter={mouseEnter} onMouseOut={mouseOut} />
    </motion.div>
  );
});
