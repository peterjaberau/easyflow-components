...
import { IExpressionShape } from './interface';
...
export function getDecoration(view: EditorView, expressions?: IExpressionShape[]) {
  ...
  expressions.forEach((expression) => {
    ...
    if (start >= 0) {
      ...
      const rang = (expression.hasError ? errorHighlightMark : defaultHighlightMark).range(start, i);
      ranges.push(rang);
    }
  });
...
const getHighlightExtDecorations = (value: { decoration: DecorationSet; update: (update: ViewUpdate) => void }) =>
  value.decoration;
...
export const getHighlightExpressionExtension = (expressions?: IExpressionShape[]) => {
