...
export const fromNow = (date: string) => {
  ...
  if (diff / PER_DAY >= 7 && dayjs(date).isSame(now, 'year')) {
    return dayjs(date).format(FORMAT_RULE[local]?.inYear || FORMAT_RULE['en-us'].inYear);
  }

  if (diff / PER_DAY >= 7 && !dayjs(date).isSame(now, 'year')) {
    return dayjs(date).format(FORMAT_RULE[local]?.otherYear || FORMAT_RULE['en-us'].otherYear);
  }
...
export default { fromNow, formatDate, ILLADayjs, initDayjs };
