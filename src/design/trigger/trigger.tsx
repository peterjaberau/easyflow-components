...
export const Trigger: FC<TriggerProps> = (props) => {
  ...
  const _popupContainer =
    (popupContainer instanceof HTMLElement ? popupContainer : popupContainer?.current) ??
    (_renderInBody ? document.body : null);
  ...
  switch (placement) {
      ...
      centerNode = (
        <div css={applyVerticalContainer(autoAlignPopupWidth)}>
          <div
            ...
            css={applyTipsText(colorScheme, maxW, withoutPadding, withoutShadow, autoAlignPopupWidth)}
          >
            {closeContent}
          </div>
          {showArrow && <TriangleTop w="8px" h="4px" containerStyle={applyTriangleStyle(colorScheme, placement)} />}
        </div>
      );
      break;
    case 'bottom':
    ...
    case 'bottom-end':
      centerNode = (
        <div css={applyVerticalContainer(autoAlignPopupWidth)}>
          {showArrow && <TriangleBottom w="8px" h="4px" containerStyle={applyTriangleStyle(colorScheme, placement)} />}
          <div
            ref={tipsContainerRef}
            css={applyTipsText(colorScheme, maxW, withoutPadding, withoutShadow, autoAlignPopupWidth)}
          >
    ...
    case 'right-end':
      centerNode = (
        <div css={applyHorizontalContainer(autoAlignPopupWidth)}>
          {showArrow && <TriangleRight w="4px" h="8px" containerStyle={applyTriangleStyle(colorScheme, placement)} />}
          <div
            ref={tipsContainerRef}
            css={applyTipsText(colorScheme, maxW, withoutPadding, withoutShadow, autoAlignPopupWidth)}
          >
    ...
    case 'left-end':
      centerNode = (
        <div css={applyHorizontalContainer(autoAlignPopupWidth)}>
          ...
          {showArrow && <TriangleLeft w="4px" h="8px" containerStyle={applyTriangleStyle(colorScheme, placement)} />}
        </div>
      );
      break;
  }
  ...
  const mergedRef = useMergeRefs([refs.setReference, (props.children as any).ref, childrenRef]);
  ...
  return (
    <>
      ...
      {!disabled && (
        <AnimatePresence>
          {finalVisible && (
            <FloatingPortal root={_popupContainer ?? childrenRef?.current ?? document.body}>
              <div
                onFocus={onFocus}
                onBlur={onBlur}
                css={[
