...
import { getSpecialThemeColor, globalColor, illaPrefix } from '../theme';
...
export function applyBadgeDot(color: string, hasChildren: boolean, hasStatus?: boolean): SerializedStyles {
  let position = ``;
...
export function applyBadgeNumberOrText(color: string, hasChildren: boolean, length: number): SerializedStyles {
...
export function applyBadgeScale(isChanged = false): SerializedStyles {
  return isChanged
    ? css`
...
export function getDotColor(count: number | ReactNode, colorScheme?: BadgeColorScheme, status?: BadgeStatus): string {
  let colorStyle;
  if (colorScheme) {
