...
import { Modal } from '../../../../design/react';
...
export const ActionGenerator: FC<ActionGeneratorProps> = function (props) {
  ...
  const [currentActionType, setCurrentActionType] = useState<ActionType | null>(defaultActionType);
  ...
  useEffect(() => {
    if (currentStep === 'createAction') {
      if (currentActionType === 'aiagent') {
        return;
      } else if (getResourceByType(currentActionType as ResourceType).length === 0) {
  ...
  const handleCancelModal = useCallback(() => {
    const element = currentStep === 'createResource' ? 'resource_configure_close' : 'resource_type_modal';
    track?.(
      ILLA_MIXPANEL_EVENT_TYPE.CLICK,
      {
        element,
      ...
      <div css={modalContentStyle}>
        {currentStep === 'select' && (
          <>
            ...
            />
            <ResourceTypeSelector onSelect={handleActionTypeSelect} filterResourceType={filterResourceType} />
          </>
        )}
        {currentStep === 'createAction' &&
          ...
          (currentActionType === 'aiagent' ? (
            <>
              <ModalHeader
                title={t('editor.action.action_list.action_generator.title.choose_resource')}
                onClickClose={handleCancelModal}
              />
              <AIAgentSelector
        ...
        {currentStep === 'createResource' && currentActionType && (
          <ResourceCreator resourceType={currentActionType as ResourceType} onBack={handleBack} />
        )}
      </div>
    </Modal>
  );
