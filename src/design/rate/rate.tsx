...
import { LikeFillIcon, StarFillIcon } from '../icon';
...
export const Rate = forwardRef<HTMLDivElement, RateProps>((props, ref) => {
  ...
  const userCharacter = useCallback(
    (index: number) => {
      let usedCharacter = typeof character === 'function' ? character(index) : character;
      if (heart) {
  ...
  const renderCharacter = useCallback(
    (index: number) => {
      const fixedValue = allowHalf ? times(+divide(mergedValue || 0, 0.5).toFixed(0), 0.5) : Math.round(mergedValue);
      ...
      return (
        <CharacterWrapper key={index} {...tooltipProps}>
          <div
            css={applyRateCharacter(disabled ?? false, readonly ?? false, animation && index + 1 < mergedValue)}
          ...
          >
            <div css={applyRateCharacterLeft(allowHalf && index + 0.5 === userIndex, !heart)} {...leftProps}>
              {userCharacter(index)}
            </div>
            <div css={applyRateCharacterRight(index + 1 <= userIndex, !heart)} {...rightProps}>
              {userCharacter(index)}
            </div>
          </div>
        </CharacterWrapper>
