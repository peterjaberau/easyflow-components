...
export function getDayjsValue(
  ...
  const getRealTime = (t: DayjsPro | string) =>
    utcOffset !== undefined || timezone ? toTimezone(formatValue(t), utcOffset, timezone) : formatValue(t);
...
export function isDayjsArrayChange(prevValue: Dayjs[] | undefined, currentValue: Dayjs[] | undefined) {
  if (currentValue === undefined && prevValue === undefined) {
    return false;
  }
  return (
...
export function toTimezone(time: Dayjs, utcOffset?: number, timezone?: string, local?: boolean): Dayjs {
...
export const isInRange = (
  ...
  if (startDate && endDate) {
    return (
      isSameTime(current, startDate) || isSameTime(current, endDate) || current.isBetween(startDate, endDate, null)
    );
  }
  return false;
};
