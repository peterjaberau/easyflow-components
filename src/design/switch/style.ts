...
export function applySwitch(colorScheme: SwitchColorScheme, checked: boolean, size: SwitchSize): SerializedStyles {
...
export function applySwitchDot(size: SwitchSize, checked: boolean, disabled: boolean): SerializedStyles {
  ...
  const dotPosition = checked
    ? `
      ...
      `
    : `left:${dotSpacing}`;
  const dotShadow = disabled ? '' : `box-shadow:0 1px 2px ${globalColor(`--${illaPrefix}-blackAlpha-05`)}`;
  return css`
    display: flex;
...
export function applySwitchText(checked: boolean, size: SwitchSize): SerializedStyles {
  const textSize =
    size === 'medium'
      ? `
          font-size: 12px;
