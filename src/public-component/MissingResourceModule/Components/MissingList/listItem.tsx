...
import { Button, PlayOutlineIcon, QuestionCircleIcon, SuccessCircleIcon } from '../../../../design/react';
import { MissingResourceContext } from '../../context';
import ResourceChoose from '../ResourceChoose';
import { ItemProps } from './itemInterface';
import { cellFontStyle, cellStyle, columnContainerStyle, statusContainerStyle, statusTextStyle } from './style';
...
export const ListItem: FC<ItemProps> = (props) => {
  const { actionIDs, resourceType, replacementResourceID, status, tutorialHref, index, handleChangePlaceInfo } = props;
  ...
  const shownNames = actionIDs.map((id) => actionIDMapAction[id].displayName).join(' ');
  ...
  return (
    <div css={columnContainerStyle}>
      ...
      <div css={cellStyle('96px')}>
        <Button leftIcon={<PlayOutlineIcon />} onClick={openTutorial} variant="outline" colorScheme="grayBlue">
