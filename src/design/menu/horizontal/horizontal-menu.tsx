...
import { DropList, DropListItem } from '../../dropdown';
import { MenuContext } from '../menu-context';
import { mergeRefs, useMergeValue } from '../../system';
import { NextIcon, PreviousIcon } from '../../icon';
import { AnimatePresence, motion } from 'framer-motion';
import useMeasure from 'react-use-measure';

export const HorizontalMenu = forwardRef<HTMLDivElement, MenuProps>((props, ref) => {
  const {
  ...
  const [finalSelectedValues, setFinalSelectedValues] = useMergeValue<string[]>([], {
  ...
  const [finalSelectedSubMenu, setFinalSelectedSubMenu] = useMergeValue<string[]>([], {
    ...
    if ('children' in item || 'subItems' in item || 'opened' in item) {
            ...
            title={
              <div
                css={applyHorizontalSubMenuItemContainer(
                  colorScheme,
                  subItem.selected ?? finalSelectedValues.some((v) => v === subItem.value),
                  subItem.disabled,
                )}
              >
                {subItem.icon && <span css={applyHorizontalSubMenuIcon(onlyShowIcon)}>{subItem.icon}</span>}
                {subItem.label && <span css={horizontalSubMenuLabel}>{subItem.label}</span>}
              </div>
            }
          />
        );
      ...
      return (
        <HorizontalSubMenu
          ...
          selected={subMenu.selected ?? finalSelectedSubMenu.some((v) => v === subMenu.value)}
          opened={subMenu.opened ?? finalOpenedSubMenuValues.some((v) => v === subMenu.value)}
          onVisibleChange={(opened) => {
            if (openedSubMenuValues === undefined) {
              if (opened) {
                if (!finalOpenedSubMenuValues.includes(subMenu.value)) {
                  setFinalOpenedSubMenuValues([...finalOpenedSubMenuValues, subMenu.value]);
                }
              } else {
                setFinalOpenedSubMenuValues(finalOpenedSubMenuValues.filter((v) => v !== subMenu.value));
              }
            }
          }}
          onClick={() => {
            ...
            const openedSet = new Set([
              ...items?.filter((i) => 'opened' in i && i.opened === true).map((i) => i.value),
        ...
        >
          <DropList
            w="100%"
            onClickItem={(value: string | number, clickedNode: ReactNode, event: MouseEvent) => {
              onClickMenuItem?.(value as string, [item.value, value as string], event);
              if (!finalSelectedValues.includes(value as string)) {
                if (selectedValues === undefined) {
                  setFinalSelectedValues([value as string]);
                }
                onMenuSelect?.(value as string, [subMenu.value, value as string], [value as string]);
              }
      ...
      return (
        <HorizontalMenuItem
          ...
          hidden={item.hidden}
          selected={item.selected ?? finalSelectedValues.some((v) => v === item.value)}
          onClick={(e) => {
  ...
  return (
    ...
    >
      ...
      >
        <div
          ref={containerRef}
          css={applyHorizontalMenuListContainerStyle(horizontalAlign, leftScroll || rightScroll)}
          onScroll={(event) => {
