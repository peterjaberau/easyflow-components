...
import { HuggingFaceEndpointResource, Resource } from '../../../../publicTypes';
import { TextLink } from '../../../../TextLink';
import { FC, useContext } from 'react';
import { useFormContext } from 'react-hook-form';
import { Trans, useTranslation } from 'react-i18next';
import { FileDefaultIcon, WarningCircleIcon } from '../../../../../design/icon';
...
import { container, errorIconStyle, errorMsgStyle, labelContainer } from '../style';

const HuggingFaceEndpointConfigElement: FC<BaseConfigElementProps> = (props) => {
  const { resourceID } = props;
  const { t } = useTranslation();

  const { control, formState } = useFormContext();
  const { getResourceByID } = useContext(ResourceGeneratorContext);
  const findResource = getResourceByID(resourceID) as Resource<HuggingFaceEndpointResource>;
      ...
      <div css={container}>
        <div css={docsItemContainerStyle}>
          ...
          <div css={docContainerStyle}>
            ...
            <span
              css={docItemStyle}
              onClick={() => handleURLClick('https://huggingface.co/docs/inference-endpoints/index')}
            >
              <FileDefaultIcon />
            ...
            <span
              css={docItemStyle}
              onClick={() => handleURLClick('https://huggingface.co/docs/api-inference/quicktour')}
            >
              <FileDefaultIcon />
              <span>API Doc</span>
            </span>
        ...
        <ControlledElement
          ...
          controlledType="input"
          control={control}
          placeholders={[t('editor.action.resource.api.placeholder.hf_endpoint')]}
          rules={[
            {
              ...
              <Trans
                ...
                components={[
                  <TextLink
                    ...
                    onClick={() => {
                      handleURLClick('https://huggingface.co/docs/inference-endpoints/guides/create_endpoint');
                    }}
                  />,
                ]}
              />
...
HuggingFaceEndpointConfigElement.displayName = 'HuggingFaceEndpointConfigElement';
