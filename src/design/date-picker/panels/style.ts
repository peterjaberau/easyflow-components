...
export const applyCellStyle = ({
  ...
  return css`
    ...
    ${(isRangeStart || isRangeEnd) && !isDisabled && !isSelected ? cellRangeStyle : ''};
...
export const applyDateValueCellStyle = ({
  ...
  return css`
    ...
    ${isHoverInRange || ((isHoverRangeStart || isHoverRangeEnd) && !isRangeStart && !isRangeEnd)
      ? dateCellHoverInRangeStyle
      : ''}
    ${(mode === 'year' || mode === 'month' || mode === 'quarter') && `width: 100%;`}
  `;
};
