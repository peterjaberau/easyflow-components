...
import { FREE_TEAM_LIMIT_TYPE, handleFreeTeamLimitError } from '../../../../UpgradeModal';
import { teamActions } from '../../../../UserData';
import { isSmallThanTargetRole } from '../../../../UserRoleUtils';
import { FC, useCallback, useContext, useEffect, useMemo } from 'react';
import { useTranslation } from 'react-i18next';
import { useDispatch } from 'react-redux';
import { Button, DropList, DropListItem, Dropdown, MoreIcon, useMessage, useModal } from '../../../../../design/react';
import { fetchChangeTeamMemberRole, fetchRemoveTeamMember } from '../../../service';
...
export const MoreAction: FC<MoreActionProps> = (props) => {
  const { currentUserRole, userRole, userStatus, teamMemberID, currentUserID, name, email, teamID } = props;
  ...
  const disabled = useMemo(() => {
    return teamMemberID === currentUserID || isSmallThanTargetRole(userRole, currentUserRole, false);
    ...
    modal.show({
      ...
      onOk: async () => {
        ...
        try {
          ...
          const res = handleFreeTeamLimitError(e, FREE_TEAM_LIMIT_TYPE.TRANSFER_OWNER);
          if (res) return;
          message.error({
  ...
  return disabled ? null : (
    <div css={moreActionWrapper}>
      <Dropdown
        ...
        dropList={
          <DropList>
            {userRole !== USER_ROLE.OWNER && userStatus !== USER_STATUS.PENDING && (
              <AuthShown currentUserRole={currentUserRole} allowRoles={[USER_ROLE.OWNER]} rules={SHOW_RULES.EQUAL}>
                <DropListItem
                  key="trans"
                  value="trans"
                  onClick={() => {
            ...
            <AuthShown currentUserRole={currentUserRole} allowRoles={[userRole]} rules={SHOW_RULES.BIGGER}>
              <DropListItem
                key="remove"
                value="remove"
                onClick={() => {
      ...
      >
        <Button variant="text" colorScheme="grayBlue" w="32px" disabled={disabled} leftIcon={<MoreIcon />} />
      </Dropdown>
    </div>
  );
};
