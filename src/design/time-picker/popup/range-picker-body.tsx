...
import { DayjsPro, isDayjs, omit, toLocal, toTimezone } from '../../system';
...
export const RangePickerBody: FC<RangeTimePickerPopupProps> = (props) => {
  ...
  const onSelectTime = useCallback(
    (_: string, time: DayjsPro) => {
      ...
      if (disableConfirm && Array.isArray(v) && isDayjs(v[0]) && isDayjs(v[1])) {
        onConfirmValue?.(zoneValue);
      }
    },
    [disableConfirm, focusedInputIndex, format, onConfirmValue, onSelect, setValueShow, timezone, utcOffset, valueShow],
  );
