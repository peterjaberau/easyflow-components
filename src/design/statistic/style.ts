...
const getStatisticColor = (color: string) => {
  ...
  } else if (color === 'blackAlpha' || color === 'gray' || color === 'grayBlue') {
...
export function applyStatisticDecoratorStyle(isPrefix: boolean, isText: boolean): SerializedStyles {
  const spacing = isText ? 4 : 8;
  const margin = isPrefix ? `margin-right:${spacing}px;` : `margin-left:${spacing}px;`;
