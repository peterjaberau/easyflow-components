...
export const CompleteCalendar = forwardRef<HTMLDivElement, CalendarProps>((props, ref) => {
  const {
    value,
    defaultValue,
    colorScheme = 'blue',
  ...
  const [selectedDate, setSelectedDate] = useMergeValue<Dayjs>(dayjs(new Date()), {
  ...
  const title = useMemo(() => {
    if (configProviderProps?.locale === zhCN || configProviderProps?.locale === koKR) {
      return `${currentDate.year()} ${locale['year']} ${
        selectType === 'month' ? `${currentDate.month() + 1} ${locale['month']}` : ''
      }`;
    } else {
  ...
  return (
    <div css={[applyCalendarContainerStyle(), applyBoxStyle(props)]} ref={ref} {...deleteCssProps(otherProps)}>
            ...
            onClick={() => {
              ...
              if (value === undefined) {
                ...
                if (
                  ...
                  (selectType === 'month' && today.month() !== currentDate.month())
                ) {
                  onPanelChange?.(today);
                }
              }
      ...
      {selectType === 'month' ? (
        <MonthBigCalendar
          ...
          onChange={(v) => {
            ...
            setCurrentDate(v);
            if (v.year() !== currentDate.year() || v.month() !== currentDate.month()) {
              onPanelChange?.(v);
            }
            onChange?.(v);
          }}
