...
import { Children, cloneElement, forwardRef, Fragment, ReactElement, useMemo } from 'react';
...
export const Breadcrumb = forwardRef<HTMLDivElement, BreadcrumbProps>((props, ref) => {
  const { separator, routes, maxCount, blockRouterChange, children, onClickPath, ...otherProps } = props;

  const separatorNode = useMemo(() => {
    return typeof separator === 'string' ? (
      <span css={dividerStyle}>{separator}</span>
    ) : (
      separator ?? <SlashIcon ml="12px" mr="12px" fs="8px" c={getColor('gray', '06')} />
    );
  }, [separator]);
    ...
    if (routes) {
                      ...
                      return (
                        <DropListItem
                          ...
                          onClick={() => {
                            ...
                            onClickPath?.(c.path ?? '', index === routes.length - 1);
                          }}
                        />
                      );
                    })}
      ...
      return Children.toArray(children)
        ...
        .map((child, index, array) => {
          return (
            <Fragment key={index}>
              {index !== array.length - 1 ? child : cloneElement(child as ReactElement, { last: true })}
  ...
  return (
    <div css={[breadcrumbContainerStyle, applyBoxStyle(props)]} {...deleteCssProps(otherProps)}>
