...
import { Button, Divider, Input, Password, WarningCircleIcon } from '../../../../design/react';
...
export const PCRegister: FC<RegisterProps> = (props) => {
  ...
  const { lockedEmail, onSubmit, errorMsg, loading, showCountDown, onCountDownChange, sendEmail, hideOAuth } = props;
  ...
  const validReport = async () => {
    ...
    if (isValid) {
      validateReport(ILLA_MIXPANEL_PUBLIC_PAGE_NAME.SIGNUP, 'create_account', true, {});
    }
  ...
  useEffect(() => {
    if (asyncValid && !asyncValid.isValid) {
      validateReport(ILLA_MIXPANEL_PUBLIC_PAGE_NAME.SIGNUP, 'create_account', false, errors);
    }
  }, [errors, asyncValid]);
  ...
  return (
    <div>
      <form css={gridFormStyle} autoComplete="off" onSubmit={handleSubmit(onSubmit)}>
        ...
        <section css={gridFormFieldStyle}>
          <section css={gridItemStyle}>
            <label css={formLabelStyle}>{t('page.user.sign_up.fields.username')}</label>
          ...
          <section css={gridItemStyle}>
            <label css={formLabelStyle}>{t('page.user.sign_up.fields.email')}</label>
            <div css={gridValidStyle}>
              <Controller
                name="email"
                control={control}
                ...
                rules={{
                  ...
                  pattern: {
                    value: EMAIL_FORMAT,
                    message: t('page.user.sign_up.error_message.email.invalid_pattern'),
                  },
                }}
              />
          ...
          {isCloudVersion && (
            <section css={gridItemStyle}>
              <label css={formLabelStyle}>{t('page.user.sign_up.fields.verification_code')}</label>
              <div css={gridValidStyle}>
                <Controller
                  ...
                  render={({ field }) => (
                    <Input
                      ...
                      placeholder={t('page.user.sign_up.placeholder.verification_code')}
                      onFocus={() => {
                        ILLAMixpanel.track(ILLA_MIXPANEL_EVENT_TYPE.FOCUS, {
                          page: ILLA_MIXPANEL_PUBLIC_PAGE_NAME.SIGNUP,
                  ...
                  rules={{
                    required: t('page.user.sign_up.error_message.verification_code.require'),
                  }}
                />
          ...
          <section css={gridItemStyle}>
            <label css={formLabelStyle}>{t('page.user.sign_up.fields.password')}</label>
            <div css={gridValidStyle}>
              <Controller
                name="password"
                control={control}
                ...
                rules={{
                  ...
                  validate: (value) => {
                    return value.includes(' ') ? t('setting.password.error_password_has_empty') : true;
                  },
                }}
              />
      ...
      {isCloudVersion && !hideOAuth && (
        <div>
          <Divider mg="24px 0" colorScheme="grayBlue" text={t('page.user.sign_in.option.or')} />
          <div css={oAuthButtonGroupStyle}>
            <OAuthButton icon={<GithubIcon css={oAuthIconStyle} />} type="github" isMobile={false} landing="signup">
