...
export const SingleCascader = forwardRef<HTMLDivElement, CascaderProps>((props, ref) => {
  const {
    size = 'medium',
    allowClear,
    placeholder,
  ...
  const [finalInputValue, setFinalInputValue] = useMergeValue('', {
    defaultValue: defaultValue === undefined ? undefined : (defaultValue as string[]).join(' / '),
    value: value === undefined ? undefined : getLabelListFromValueList(value as string[], options ?? []),
  });

  const [finalValue, setFinalValue] = useMergeValue([], {
    defaultValue: defaultValue === undefined ? undefined : (defaultValue as string[]),
