...
import { forwardRef, useImperativeHandle, useRef, KeyboardEvent, useCallback, ChangeEvent } from 'react';
import React from 'react';
import { ClearIcon } from '../../icon';
import { omit } from '../../system';
...
export const RangeDateInput = forwardRef<DateInputRangeHandler, DateInputRangeProps>((props, ref) => {
  const {
    allowClear,
    error,
    disabled,
  ...
  return (
    <div
      ...
      css={applyInputWrapperStyle(size, colorScheme, disabled1 && disabled2, error, allowClear && value.length === 2)}
    >
      {prefix && <div css={applyPrefixStyle(size)}>{prefix}</div>}
      <div css={applyRangeInputStyle(!!popupVisible && focusedInputIndex === 0)}>
        <input
          ref={input0}
          disabled={disabled1}
      ...
      <div css={applyRangeInputStyle(!!popupVisible && focusedInputIndex === 1)}>
        <input
          ref={input1}
          disabled={disabled2}
      ...
      <div css={baseSuffixStyle}>
        ...
        <span className="suffix-icon" css={suffixIconStyle(disabled1 && disabled2)}>
          {suffixIcon}
        </span>
      </div>
    </div>
