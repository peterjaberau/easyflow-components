...
import { useEffect, useRef, useState, MouseEvent, ChangeEvent, forwardRef } from 'react';
import {
  getDayjsValue,
  toLocal,
  toTimezone,
...
export const RangePicker = forwardRef<HTMLDivElement, RangePickerProps>((props, ref) => {
  const {
    allowClear = true,
    disableConfirm,
    placeholder,
  ...
  const [value, setValue] = useState<Dayjs[] | undefined>(
    getDefaultValue(_format, props.value, props.defaultValue, utcOffset, timezone) as Dayjs[],
  );
  ...
  useEffect(() => {
    if (isDidMount.current) {
      if (value && (previousUtcOffset !== utcOffset || timezone !== previousTimezone)) {
        ...
        const zoneValue = localValue.map((lc) => toTimezone(lc, utcOffset, timezone));
        setValue(zoneValue);
      }
    } else {
  ...
  const onConfirmValue = (vs?: Dayjs[]) => {
    const newValue = order && Array.isArray(vs) ? getSortedDayjsArray(vs.map((v) => getFormatTime(v))) : vs;
    setValue(newValue);
    setValueShow(undefined);
    setInputValue(undefined);
  ...
  const confirmInputValue = (newInputValue?: string) => {
    ...
    const newValueShow = [...(Array.isArray(valueShow) ? valueShow : (value as Dayjs[]) || [])];
