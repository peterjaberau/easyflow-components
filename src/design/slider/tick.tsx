...
export const Tick: FC<SliderTick> = (props) => {
  const { left, leftValue, rightValue, currentWidth, disabled, colorScheme, value } = props;
  const rightBound = currentWidth - rightValue >= left;
  const background = applyBgColor(colorScheme, !(leftValue <= left && rightBound && !disabled));
  return (
