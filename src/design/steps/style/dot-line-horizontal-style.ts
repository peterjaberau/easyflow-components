...
export function applyHorizontalDescriptionStyle(type?: StepsType): SerializedStyles {
  return css`
    margin-right: 16px;
    white-space: pre;
...
export function applyHorizontalLineStyle(status: StepsStatus, lineless?: boolean): SerializedStyles {
  let c = css``;
  switch (status) {
    case 'wait':
      c = css`
