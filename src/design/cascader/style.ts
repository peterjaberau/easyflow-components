...
export function applyListSelectorSingleItemStyle(selected: boolean): SerializedStyles {
  return css`
    padding: 9px 16px;
    display: flex;
    flex-direction: row;
...
export function applyListSelectorItemTextStyle(checked: boolean): SerializedStyles {
  return css`
    color: ${getColor('grayBlue', checked ? '02' : '03')};
  `;
}

export function applySelectorGroupStyle(showRightBorder: boolean): SerializedStyles {
  let borderStyle = css``;
  if (showRightBorder) {
    borderStyle = css`
