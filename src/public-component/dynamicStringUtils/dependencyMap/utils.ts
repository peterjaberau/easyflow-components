...
export function sortDependencies(dependencyMap: DependencyMap): SortDependencies {
  ...
  try {
    ...
    const cyclicNodes = (error as Error).message.match(new RegExp('Cyclic dependency, node was:"(.*)"'));
    const node = cyclicNodes ? cyclicNodes[1] : '';
    return { success: false, cyclicNode: node, error };
  }
}

const makeParentsDependOnChild = (dependencyMap: DependencyMap, child: string) => {
  let curKey = child;
  let matches: string[] | null;

  while ((matches = curKey.match(IMMEDIATE_PARENT_REGEX)) !== null) {
    const immediateParent = matches[1];
    const existingImmediateParentDeps = dependencyMap.getDirectDependencies(immediateParent) || [];
 ...
 * }
 */
export const extractInfoFromBinding = (script: string, allKeys: Record<string, true>) => {
  const { references } = extractIdentifierInfoFromCode(script);
  return getPrunedReferences(references, allKeys);
};

export const getPrunedReferences = (references: string[], allKeys: Record<string, true>) => {
...
export const extractInfoFromBindings = (bindings: string[], allKeys: Record<string, true>) => {
