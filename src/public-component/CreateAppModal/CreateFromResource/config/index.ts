...
import { BuildAppInfo, DatasetTableRows, FORM_VIEW, RESOURCE_TYPE } from '../interface';
import {
  buildAddButton,
  buildDataGrid,
  buildDivider,
...
export const buildAppWithResourceSchema = (resource: RESOURCE_TYPE, rows: DatasetTableRows[]): BuildAppInfo => {
  ...
  const { editInputs, addInputs } = getNodesAboutForm(rows.filter(({ isSelected }) => isSelected));
  ...
  editNode.childrenNode.push(...editInputNodes, editDividerNode, editSaveButtonNode, editDelButtonNode);
  ...
  const editInputDisplayNames = editInputs.map((inputNode) => inputNode.displayName);
  const addInputDisplayNames = addInputs.map((inputNode) => inputNode.displayName);
