...
import { UploadProps, UploadItem, STATUS, UploadInstance, UploaderInstance } from './interface';
import Uploader from './uploader';
import { isFunction, isNumber, isArray } from '../system';
...
export const Upload = forwardRef<UploadInstance, UploadProps>((props, ref) => {
  ...
  uploadStateRef.current = 'fileList' in props ? getUploadState(props.fileList) : innerUploadState;
  ...
  const removeFile = useCallback(
    (file: UploadItem) => {
      ...
      Promise.resolve(isFunction(onRemove) ? onRemove(file, getFileList(uploadStateRef.current)) : onRemove)
        .then((val) => {
          if (val !== false) {
    ...
    return {
      submit: (file?: UploadItem) => {
        const list: UploadItem[] =
          (file ? [file] : getFileList(uploadStateRef.current).filter((x) => x.status === STATUS.INIT)) || [];
        list.forEach((x) => {
          uploadFile(x);
        });
      },
  ...
  const disabledUploadDom = props.disabled ?? (!limit.hideOnExceedLimit && exceedLimit);
  ...
  const handleFileStatusChange = useCallback(
    (file: UploadItem) => {
      ...
      onChange && onChange(getFileList({ ...uploadStateRef.current, [file.uid]: file }), file);
    },
    [onChange, props],
  );
