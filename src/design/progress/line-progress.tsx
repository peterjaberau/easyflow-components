...
import { SuccessIcon, WarningCircleIcon } from '../icon';
...
export const LineProgress = forwardRef<HTMLDivElement, ProgressProps>((props, ref) => {
  ...
  switch (status) {
    case 'success':
      statusComponent = <SuccessIcon color={globalColor(`--${illaPrefix}-green-03`)} />;
      break;
    case 'error':
      statusComponent = <WarningCircleIcon color={globalColor(`--${illaPrefix}-red-03`)} />;
      break;
  }

  if (steps == 1) {
    return (
      <div ref={ref} css={[applyProgressContainer, applyBoxStyle(props)]} {...deleteCssProps(otherProps)}>
        ...
        {showText && <span css={applyProgressText}>{formatText(percent)}</span>}
        {status != 'normal' && <span css={applyStatusIcon}>{statusComponent}</span>}
      </div>
    );
  } else {
    const lineProgressContainer = [];
    const lineProgressBgContainer = [];
    ...
    for (let i = 0; i < steps; i++) {
      ...
      lineProgressBgContainer.push(
        <div
          key={i}
          css={applyLineProgressBgStep(strokeWidth, `calc((${w} - (${steps} - 1) * 4px) / ${steps})`, trailColor)}
        />,
      );
    }
