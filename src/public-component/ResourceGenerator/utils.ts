import { isBlobURLOrUrl } from '../utils';
import { getI18n, useTranslation } from 'react-i18next';

export function getResourceNameFromResourceType(resourceType: string | null): string {
  switch (resourceType) {
    case 'supabasedb':
      return 'Supabase DB';
    case 'mysql':
...
export const validateNotEmpty = (value?: string) => value != undefined && value.trim() != '';
...
export const urlValidate = (value: string) => {
  return isBlobURLOrUrl((value ?? '').trim()) ? true : getI18n().t('editor.action.resource.error.invalid_url');
};
