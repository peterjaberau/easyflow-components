...
export const VerticalMenu = forwardRef<HTMLDivElement, MenuProps>((props, ref) => {
  const {
        ...
        return (
          <VerticalMenuItem
            ...
            onClick={(e) => {
              ...
              onClickMenuItem?.(subItem.value, [subMenu.value, subItem.value], e);
              if (!finalSelectedValues.includes(subItem.value)) {
                if (selectedValues === undefined) {
                  setFinalSelectedValues([subItem.value]);
                }
                onMenuSelect?.(subItem.value, [subMenu.value, subItem.value], [subItem.value]);
              }
            ...
            label={
              ...
              >
                {subItem.icon && <span css={verticalSubMenuIcon}>{subItem.icon}</span>}
                {subItem.label && <span css={verticalSubMenuLabel}>{subItem.label}</span>}
              </div>
            }
          />
        );
  ...
  return (
    ...
    >
      <div css={[verticalMenuContainerStyle, applyBoxStyle(otherProps)]} ref={ref} {...deleteCssProps(otherProps)}>
        {c}
      </div>
    </MenuContext.Provider>
  );
