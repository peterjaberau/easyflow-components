...
export function applyChildrenContainerStyle(active?: boolean): SerializedStyles {
  return css`
...
export function applyCollapseTitleContainerStyle(disabled?: boolean): SerializedStyles {
  return css`
    display: flex;
    align-items: center;
    padding: 8px 12px;
...
export function applyCollapseTitleStyle(showIcon: boolean, disabled?: boolean): SerializedStyles {
  return css`
...
export function applyPositionIconAnimateStyle(active: boolean, position?: CollapsePosition): SerializedStyles {
  return css`
    transform: rotate(${active ? (position === 'left' ? '90deg' : '-90deg') : '0deg'});
