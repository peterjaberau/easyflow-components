  ...
  return {
    ...
    unSubscribe: (listenerId) => {
      store.listener.splice(
        store.listener.findIndex((listener) => listener.listenerId === listenerId),
        1,
      );
    },
    add: (Notification) => {
    ...
    remove: (NotificationId) => {
      const index = store.NotificationList.findIndex((Notification) => Notification.id === NotificationId);
      if (index != -1) {
