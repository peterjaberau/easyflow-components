...
import { AddIcon, Select, SelectOptionObject, Space } from '../../../../design/react';
import { MissingResourceContext } from '../../context';
import { agentActionStyle, createNewStyle, itemContainer, itemLogo, itemText } from './style';
...
const ResourceChoose: FC<ResourceChooseProps> = ({
  ...
  const { resourceList, aiAgentList, addResourceCallback } = useContext(MissingResourceContext);
  const { track }: any = useContext(MixpanelTrackContext);

  const finalResourceList = getSameTypeResourceList(resourceList, origanResourceType);
  const [generatorVisible, setGeneratorVisible] = useState(false);
  const isMysqlLike = LIKE_MYSQL_TYPES.includes(origanResourceType);
  const isAIAgent = origanResourceType === 'aiagent';

  const createOrUpdateResourceCallback = (resource: Resource, isUpdate: boolean) => {
  ...
  const options: SelectOptionObject[] = isAIAgent
    ...
    : finalResourceList.map((item) => ({
        label: (
          <div css={itemContainer}>
            <span css={itemLogo}>
              <Suspense fallback={<div>Loading...</div>}>{getIconFromResourceType(item.resourceType, '14px')}</Suspense>
            </span>
  ...
  if (!isAIAgent) {
    options.unshift({
      label: (
        <Space size="8px" direction="horizontal" alignItems="center" css={createNewStyle}>
          <AddIcon size="16px" />
  ...
  return (
    <>
      <Select
        ...
        options={options}
        value={resourceID || t('editor.action.panel.label.placeholder.missing_resource.choose_a_resource')}
        onChange={(value) => {
