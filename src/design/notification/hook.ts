import { v4 } from 'uuid';
import { NotificationGlobalProps, NotificationHandler, notificationStore } from './notification-store';
import { NotificationProps } from './interface';

const showImpl = (notification: NotificationProps) => {
  if (notificationStore.getNotification().length > (notificationStore.getConfig().maxCount ?? 5)) {
