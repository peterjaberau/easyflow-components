...
import { forwardRef, MutableRefObject, useCallback, useMemo, useRef, useState, FocusEvent } from 'react';
import { InputNumberProps } from './interface';
import { Input } from '../input';
import { DownIcon, MinusIcon, PlusIcon, UpIcon } from '../icon';
import { Space } from '../space';
import { isNumber, mergeRefs } from '../system';
import { applyActionIconStyle, applyControlBlockStyle, controlContainerStyle, hoverControlStyle } from './style';
...
export const InputNumber = forwardRef<HTMLInputElement, InputNumberProps>((props, ref) => {
  ...
  const [innerValue, setInnerValue] = useState(() => {
    return getDecimal('value' in props ? props.value! : 'defaultValue' in props ? defaultValue! : '');
  });
  ...
  const setValue = useCallback(
    (newValue: BigIntDecimal | NumberDecimal) => {
      ...
      if (!newValue.equals(value) && onChange) {
        ...
        onChange(newValue.isEmpty ? undefined : newValue.isNaN ? NaN : Number(newValueStr));
      }
    },
    [mergedPrecision, onChange, value],
  );

  const getLegalValue = useCallback<(value: BigIntDecimal | NumberDecimal) => BigIntDecimal | NumberDecimal>(
    (changedValue) => {
  ...
  const currentInputRef = useRef<HTMLInputElement>() as MutableRefObject<HTMLInputElement>;
  ...
  return (
    <Input
      ...
      suffix={
        <Space mr="-8px">
          {suffix}
          {!hideControl && !readOnly && !disabled && mode === 'embed' && control}
        </Space>
      }
      addBefore={
        mode === 'button' ? (
