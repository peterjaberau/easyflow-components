import { SUBSCRIBE_PLAN, TeamInfo, USER_ROLE } from '../publicTypes';
import { getPlanUtils } from '../UserData';
...
export const USER_ROLE_ARRAY = [USER_ROLE.OWNER, USER_ROLE.ADMIN, USER_ROLE.EDITOR, USER_ROLE.VIEWER, USER_ROLE.GUEST];
export const filterUserRole = (roles: USER_ROLE[], filterRole: USER_ROLE[] = []) => {
  if (Array.isArray(filterRole) && filterRole.length > 0) {
    return roles.filter((role) => !filterRole.includes(role));
  }
  return roles;
};
export const getSmallThanTargetRole = (targetRole: USER_ROLE, notHasSelf = true, filterRole: USER_ROLE[] = []) => {
  const targetRoleIndex = USER_ROLE_ARRAY.indexOf(targetRole);
  const result = notHasSelf ? USER_ROLE_ARRAY.slice(targetRoleIndex + 1) : USER_ROLE_ARRAY.slice(targetRoleIndex);
  return filterUserRole(result, filterRole);
};

export const getBiggerThanTargetRole = (targetRole: USER_ROLE, notHasSelf = true, filterRole: USER_ROLE[] = []) => {
  const targetRoleIndex = USER_ROLE_ARRAY.indexOf(targetRole);
  const result = notHasSelf ? USER_ROLE_ARRAY.slice(0, targetRoleIndex) : USER_ROLE_ARRAY.slice(0, targetRoleIndex + 1);
  return filterUserRole(result, filterRole);
};

export const isSmallThanTargetRole = (targetRole: USER_ROLE, currentUserRole: USER_ROLE, isEqual = true) => {
  const targetRoleIndex = USER_ROLE_ARRAY.indexOf(targetRole);
  ...
  return isEqual ? currentUserRoleIndex >= targetRoleIndex : currentUserRoleIndex > targetRoleIndex;
};

export const isBiggerThanTargetRole = (targetRole: USER_ROLE, currentUserRole: USER_ROLE, isEqual = true) => {
  const targetRoleIndex = USER_ROLE_ARRAY.indexOf(targetRole);
  const currentUserRoleIndex = USER_ROLE_ARRAY.indexOf(currentUserRole);
...
export const getAttribute = (
  ...
  switch (teamPlan) {
    ...
    case SUBSCRIBE_PLAN.TEAM_LICENSE_INSUFFICIENT: {
      return {
        accessAttribute:
          InvalidedSubscribePlanAttributeConfigList?.[ATTRIBUTE_CATEGORY.ACCESS]?.[userRole]?.[attributeGroup],
        deleteAttribute:
          InvalidedSubscribePlanAttributeConfigList?.[ATTRIBUTE_CATEGORY.DELETE]?.[userRole]?.[attributeGroup],
        manageAttribute:
          InvalidedSubscribePlanAttributeConfigList?.[ATTRIBUTE_CATEGORY.MANAGE]?.[userRole]?.[attributeGroup],
        specialAttribute:
          InvalidedSubscribePlanAttributeConfigList?.[ATTRIBUTE_CATEGORY.SPECIAL]?.[userRole]?.[attributeGroup],
      };
    }
    default: {
      return {
...
export const canAccess = (
  ...
  const accessAttribute = getAttribute(userRole, attributeGroup, teamPlan).accessAttribute;
...
export const canDelete = (
  ...
  const deleteAttribute = getAttribute(userRole, attributeGroup, teamPlan).deleteAttribute;
...
export const canManage = (
  ...
  const manageAttribute = getAttribute(userRole, attributeGroup, teamPlan).manageAttribute;
...
export const canAccessManage = (
  ...
  const accessAttribute = getAttribute(userRole, ATTRIBUTE_GROUP.BILLING, teamPlan).accessAttribute;
...
export const canManagePayment = (
  ...
  const attribute = isSubscribe ? ACTION_MANAGE.PAYMENT_INFO : ACTION_MANAGE.PAYMENT;

  const manageAttribute = getAttribute(userRole, ATTRIBUTE_GROUP.BILLING, teamPlan).manageAttribute;
...
export const canManageSpecial = (
  ...
  const specialAttribute = getAttribute(userRole, attributeGroup, teamPlan).accessAttribute;
  return !!specialAttribute?.[attribute];
};

export const canInvite = (userRole: USER_ROLE, teamPlan: SUBSCRIBE_PLAN = SUBSCRIBE_PLAN.TEAM_LICENSE_FREE) => {
  const attribute = inviteRoleAttributeMap[userRole];
  if (!attribute) {
    return false;
  }
  const accessAttribute = getAttribute(userRole, ATTRIBUTE_GROUP.INVITE, teamPlan).accessAttribute;
  return !!accessAttribute?.[attribute];
};
...
export const canModifyRoleFromTo = (
  ...
  const manageAttribute = getAttribute(userRole, attributeGroup, teamPlan).accessAttribute;
...
export const showShareAppModal = (
  ...
  if (canInvite) {
    return true;
  } else if (canManage(userRoleForThisApp, ATTRIBUTE_GROUP.APP, getPlanUtils(teamInfo), ACTION_MANAGE.EDIT_APP)) {
    return true;
...
export const showShareAgentModal = (teamInfo: TeamInfo, userRoleForThisAgent: USER_ROLE, isContributed: boolean) => {
  const canInvite = canManageInvite(
    teamInfo.myRole,
    teamInfo.permission.allowEditorManageTeamMember,
    teamInfo.permission.allowViewerManageTeamMember,
...
export const openShareAgentModal = (teamInfo: TeamInfo, userRoleForThisAgent: USER_ROLE, isContributed: boolean) => {
  if (isContributed) {
    return true;
  } else if (
    canManage(userRoleForThisAgent, ATTRIBUTE_GROUP.AI_AGENT, getPlanUtils(teamInfo), ACTION_MANAGE.CREATE_AI_AGENT)
  ) {
    return true;
  } else {
    return canManageInvite(
...
export const canCreateApp = (
  ...
  const manageAttribute = getAttribute(userRole, ATTRIBUTE_GROUP.APP, teamPlan).manageAttribute;
