...
import { isObject, isFunction } from '../../system';
...
const TextItem = (props: UploadListItemProps) => {
  ...
  const showUploadList = isObject(props.showUploadList) ? (props.showUploadList as CustomIconType) : {};
  ...
  const imageNode = imageUrl && isImageUrl(file) ? <img src={imageUrl} /> : <Icon />;
  ...
  return (
    <div css={getTextItemContainerStyle(props.listType)}>
      <div css={getTextItemContentContainerStyle(props.listType)}>
        {props.listType === 'picture-list' && <div css={textItemImageStyle}>{pictureListIcon}</div>}
        {props.listType === 'text' && actionIcons.fileIcon !== null && (
          <span css={textItemIconStyle}>{actionIcons.fileIcon || <Icon />}</span>
        )}
        <div css={textItemMainContentStyle}>
          {imageUrl ? (
            <a href={imageUrl} target="_blank" rel="noreferrer" css={getTextItemNameStyle(file.status)}>
              {fileName}
            </a>
          ) : (
