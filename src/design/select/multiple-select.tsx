...
import { Dropdown, DropList, DropListItem } from '../dropdown';
import { isObject, useMergeValue } from '../system';
import { DownIcon, LoadingIcon, UpIcon } from '../icon';
import { getColor } from '../theme';
import { Empty } from '../empty';
import { InputTag, InputTagRefHandler, TagObject } from '../input-tag';
import { Checkbox } from '../checkbox';
import { dropLabelStyle, dropListItemStyle } from './style';
import React from 'react';

export const MultipleSelect = forwardRef<HTMLDivElement, SelectProps>((props, ref) => {
  const {
    size = 'medium',
    allowClear,
    placeholder,
  ...
  const [finalValue, setFinalValue] = useMergeValue<string[] | SelectOptionObject[] | number[]>([], {
    value: labelInValue ? (value as SelectOptionObject[]) : (value as []),
    defaultValue: labelInValue ? (defaultValue as SelectOptionObject[]) : (defaultValue as []),
  });
  ...
  const finalOptions: SelectOptionObject[] = useMemo(() => {
    ...
    if (inputAsOption && finalInputValue && finalInputValue !== '') {
      const optionIndex = newOptions.findIndex((option) => option.value === finalInputValue);
      if (optionIndex !== -1) {
    ...
    if ((filterOption || showSearch) && finalInputValue && finalInputValue !== '') {
      newOptions = newOptions.filter((option) => {
        ...
        return typeof option.label === 'string' && option.label.includes(finalInputValue.toString());
      });
    } else {
      newOptions = newOptions.filter((option) => {
    ...
    return newOptions;
  }, [defaultFilterOption, filterOption, finalInputValue, finalValue, inputAsOption, options, showSearch]);
  ...
  const finalTagValue: TagObject[] = useMemo(() => {
    return finalValue.map((v, index) => {
      if (labelInValue) {
        ...
        return {
          label: (finalOptions as SelectOptionObject[]).find((item) => item.value === v)?.label ?? v,
          value: v,
          closeable: !readOnly,
        } as TagObject;
      }
  ...
  return (
      ...
      dropList={
        <DropList
          ...
          onClickItem={(key) => {
            ...
            if (option) {
              if (labelInValue) {
                const selectedValue = (finalValue as SelectOptionObject[]).find((v) => v.value === option.value);
                if (selectedValue !== undefined) {
                  onDeselect?.(selectedValue as SelectOptionObject);
                  const newList = [...finalValue];
                  newList.splice(
                ...
                const selectedValue = (finalValue as []).find((v) => v === option.value);
                if (selectedValue !== undefined) {
              ...
              <DropListItem
                ...
                selected={
                  typeof finalValue[0] === 'object'
                    ? (finalValue as SelectOptionObject[]).find((i) => i.value === option.value) !== undefined
              ...
              >
                <div css={dropListItemStyle}>
                  <Checkbox
                    ...
                    checked={
                      labelInValue
                        ? (finalValue as SelectOptionObject[]).find((v) => v.label === option.label) !== undefined
                        : (finalValue as []).find((v) => v === option.value) !== undefined
                    }
                  />
    ...
    >
      <InputTag
        ...
        onRemove={(v) => {
          const removedTag = v as TagObject;
          if (labelInValue) {
            const newList = [...finalValue] as SelectOptionObject[];
            const index = newList.findIndex((item) => removedTag.label === item.label);
            ...
            const newList = [...finalValue] as [];
            const index = newList.findIndex((item) => removedTag.value === item);
            newList.splice(index, 1);
            if (value === undefined) {
              setFinalValue(newList as []);
            }
        ...
        onAdd={(v, index) => {
          const addTag = v as TagObject;
          if (labelInValue) {
            const findIndex = (finalValue as SelectOptionObject[]).findIndex((value) => value.value === addTag.value);

            if (findIndex === -1) {
              const newList = [...finalValue.slice(0, index), addTag, ...finalValue.slice(index, finalValue.length)];
              if (value === undefined) {
                setFinalValue(newList as SelectOptionObject[]);
              }
              onSelect?.({
            ...
            const findIndex = (finalValue as []).findIndex((value) => value === addTag.value);
