...
import { isFunction, useMergeValue } from '../system';
import { ConfigProviderContext, ConfigProviderProps, def } from '../config-provider';
import { MoreIcon, NextIcon, PreviousIcon } from '../icon';
...
export const Pagination = forwardRef<HTMLDivElement, PaginationProps>((props, ref) => {
  ...
  const totalComponent = useMemo(() => {
    if (isFunction(showTotal)) {
      ...
      return <>{showTotal && <span css={totalStyle}>{locale.total.replace('{0}', total?.toString() ?? '')}</span>}</>;
    }
  ...
  }, [changeCurrent, disabled, inputBorderColorScheme, jumperValue, locale.go, showJumper, size, totalPageSize]);

  const clickNodeShow = useCallback(
    (index: number) => {
      let show = false;

      if (finalCurrent + bufferSize >= index && finalCurrent - bufferSize <= index) {
        show = true;
      }
  ...
  const selectorComponent = useMemo(() => {
    ...
    if (simple) {
      middleComponent = (
        <div css={selectorContainerStyle}>
          {disableSimplePageJump ? (
            <span css={applySimpleTextStyle(css``, disabled)}>{simpleValue}</span>
          ) : (
            <input
          ...
          <span css={applySimpleTextStyle(css``, disabled)}>{totalPageSize}</span>
        </div>
      );
    } else {
        ...
        if (clickNodeShow(index)) {
          ...
          if (clickNodeShow(index - 1))
            clickList.push(
              ...
              >
                {itemRender?.(finalPageSize, 'more', icons?.more ?? <MoreIcon />) ?? <MoreIcon />}
              </span>,
            );
        }
      }
    ...
    return (
      <div css={selectorContainerStyle}>
        ...
        >
          {itemRender?.(finalPageSize, 'prev', icons?.prev ?? <PreviousIcon />) ?? <PreviousIcon />}
        </span>
        {middleComponent}
        {showMore && (
          <span
        ...
        >
          {itemRender?.(finalPageSize, 'next', icons?.next ?? <NextIcon />) ?? <NextIcon />}
        </span>
      </div>
    );
  }, [
  ...
  return (
    <div css={[paginationContainer, applyBoxStyle(props)]} ref={ref} {...deleteCssProps(otherProps)}>
      {totalComponent}
      {selectorComponent}
      {pageSizeComponent}
      {jumperComponent}
