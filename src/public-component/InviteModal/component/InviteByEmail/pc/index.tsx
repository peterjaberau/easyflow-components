...
import { EMAIL_FORMAT, isCloudVersion } from '../../../../utils';
import { FC, useContext, useState } from 'react';
import { useTranslation } from 'react-i18next';
import { Button, InputTag, getColor, useMergeValue, useMessage } from '../../../../../design/react';
...
export const InviteByEmailPC: FC<InviteByEmailProps> = (props) => {
  ...
  return (
    <div css={inviteByEmailContainerStyle}>
      <div css={inviteByEmailLabelStyle}>{t('user_management.modal.email.invite_title')}</div>
      <div css={inviteByEmailInputContainerStyle}>
        ...
        <Button
          ...
          onClick={async () => {
            ...
            if (isBiggerThanTargetRole(USER_ROLE.EDITOR, inviteUserRole) && currentBalance < currentValue.length) {
              upgradeModal({
            ...
            for (let i = 0; i < currentValue.length; i++) {
              try {
                ...
                const currentIndex = finalInviteUserList.findIndex((item) => item.email === currentValue[i]);
                const user = {
      ...
      {isCloudVersion && (
        <div css={licenseContainerStyle}>
          ...
          <div css={applyLicenseNumberStyle(currentBalance > 0)}>{currentBalance}</div>
        </div>
      )}
