...
import { getNow, getSortedDayjsArray, isArray, isInRange } from '../../system';
...
export const getDependency = (rangeValues?: DatePickerValue[], valueShowHover?: DatePickerValue[]) => {
  ...
  const sortedRangeValues =
    selectedLength !== 2 && hoverLength === 2 ? getSortedDayjsArray(valueShowHover as Dayjs[]) : rangeValues;
  const sortedHoverRangeValues = selectedLength === 2 ? getSortedDayjsArray(valueShowHover as Dayjs[]) : [];
...
export const getCellStatus = (
  ...
  const { sortedRangeValues, sortedHoverRangeValues } = getDependency(rangeValues, valueShowHover);
  ...
  const selected = value && isSameTime(cellDateObj.time as Dayjs, value as Dayjs);
  let isToday = isSameTime(cellDateObj.time as Dayjs, getNow(utcOffset, timezone));
  ...
  const isRangeStart = getIsRangeStartOrEnd(rangeStart, checkIsInView, disabled, cellDateObj.time as Dayjs, isSameTime);
  const isRangeEnd = getIsRangeStartOrEnd(rangeEnd, checkIsInView, disabled, cellDateObj.time as Dayjs, isSameTime);
  ...
  return {
    ...
    isInRange: checkIsInView && !disabled && isInRange(cellDateObj.time as Dayjs, isSameTime, rangeStart, rangeEnd),
    isHoverRangeStart: !!isHoverRangeStart,
    isHoverRangeEnd: !!isHoverRangeEnd,
    isHoverInRange:
      checkIsInView && !disabled && isInRange(cellDateObj.time as Dayjs, isSameTime, hoverRangeStart, hoverRangeEnd),
