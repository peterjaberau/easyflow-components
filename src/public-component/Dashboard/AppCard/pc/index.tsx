...
import { ACTION_MANAGE, ATTRIBUTE_GROUP, canManage } from '../../../UserRoleUtils';
import { getAuthToken, getILLABuilderURL, isCloudVersion } from '../../../utils';
import { fromNow } from '../../../utils';
import { FC, useCallback, useContext, useMemo } from 'react';
import { useTranslation } from 'react-i18next';
import { useSelector } from 'react-redux';
import { useParams } from 'react-router-dom';
import { Space, Tag } from '../../../../design/react';
...
export const PCAppCard: FC<PCAppCardProps> = (props) => {
  ...
  const onClickCard = useCallback(() => {
    ...
    if (canEditApp) {
      window.open(
        `${getILLABuilderURL(window.customDomain)}/${teamIdentifier}/app/${appInfo.appId}?token=${getAuthToken()}`,
        '_blank',
      );
    } else if (appInfo.deployed) {
      window.open(
        `${getILLABuilderURL(window.customDomain)}/${teamIdentifier}/deploy/app/${
          appInfo.appId
        }?token=${getAuthToken()}`,
        '_blank',
      );
  ...
  return (
    <div css={cardStyle} onClick={onClickCard}>
      ...
      <div>
        <div css={descriptionStyle}>{appInfo.config.description || t('new_dashboard.desc.no_description')}</div>
      </div>
      {isCloudVersion ? (
        <>
          {editors}
          <div css={footerStyle}>
            ...
            <ActionButtonGroup appID={appInfo.appId} appDeployed={appInfo.deployed} canEditApp={canEditApp} />
          </div>
        </>
      ) : (
        <div css={footerStyle}>
