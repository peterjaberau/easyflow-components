...
export const InsufficientNoticeModal: FC<InsufficientNoticeModalProps> = (props) => {
  const { visible, configType = 'add-license', onCancel, afterClose } = props;
  const { t } = useTranslation();

  const { title, description, buttonText }: any = useMemo(() => {
    return INSUFFICIENT_MODAL_CONFIG_KEY[configType] ?? ({} as any);
  }, [configType]);
