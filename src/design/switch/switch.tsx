...
import { applySwitch, applySwitchDot, applySwitchText, applySwitchIcon } from './style';
...
export const Switch = forwardRef<HTMLButtonElement, SwitchProps>((props, ref) => {
  ...
  return (
    <button
      css={[applySwitch(colorScheme, mergedChecked, size), applyBoxStyle(props)]}
      ref={ref}
    ...
    >
      <div css={applySwitchDot(size, mergedChecked, disabled ?? false)}>
        {(checkedIcon || uncheckedIcon) && (
          <div css={applySwitchIcon}>{mergedChecked ? checkedIcon : uncheckedIcon}</div>
        )}
      </div>
      <div css={applySwitchText(mergedChecked, size)}>
        {isObject(renderText) ? <div css={applySwitchIcon}>{renderText}</div> : renderText}
      </div>
    </button>
  );
});
