...
import { useEffect, useRef, useState, forwardRef, useMemo, MouseEvent, useImperativeHandle, useCallback } from 'react';
import { Bar } from './bar';
...
import { Trigger, TriggerRefHandler } from '../trigger';
...
export const Slider = forwardRef<ICustomRef, SliderProps>((props, ref) => {
  ...
  const dragging = (x: number, startValue: number | number[], location: BarLocation) => {
    onDragging(x, startValue, location);
  };

  const dragEnd = (x: number, startValue: number | number[], location: BarLocation) => {
  ...
  }, [isRange, max, min, step, initOffsetFromState, leftValue, rightValue, width]);
  return (
    <div ref={currentRef} css={[applySliderWrapper, applyBoxStyle(props)]}>
      <div css={applySliderRoad()} ref={roadRef} onClick={tickClick}>
        ...
        {startMarkShow && <div css={applyBoundBar(disabled, false)} onClick={() => onClickTick(min)} />}
        {Boolean(isRange) && (
          <Trigger
            content={Array.isArray(currentValue) ? formatTooltip(currentValue[0]) : formatTooltip(currentValue)}
        ...
        <Trigger
          content={Array.isArray(currentValue) ? formatTooltip(currentValue[1]) : formatTooltip(currentValue)}
        ...
        {endMarkShow && <div css={applyBoundBar(disabled, true)} onClick={() => onClickTick(max)} />}
        <Bar
          width={barLength}
          left={leftOffset}
          isRange={isRange}
