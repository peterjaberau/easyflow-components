...
export interface SliderProps extends Omit<HTMLAttributes<HTMLDivElement>, SpecialSliderProps>, BoxProps {
  disabled?: boolean;
  showTicks?: boolean;
...
export interface SliderMarkBar {
  ...
  dragEnd: (x: number, startValue: number | number[], location: BarLocation) => void;
  step: number;
  partLength: number;
  min: number;
  max: number;
  ...
  right: number;
  colorScheme?: SliderColorScheme;
  drag: (x: number, startValue: number | number[], location: BarLocation) => void;
  value: number | number[];
...
export interface IUseOffsetReturn {
  ...
  barLength: number;
  partLength: number;
  initOffsetFromState: (partLength: number, width: number, rightVal: number, leftVal?: number) => void;
  onDragging: (x: number, startValue: number | number[], location: BarLocation) => void;
  onDragEnd: (
    x: number,
  ...
  onDragBarEnd: (x: number, startValue: number[], onAfterChange?: ((v: number[]) => void) | undefined) => void;
}
