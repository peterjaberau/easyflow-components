...
import { isFunction, throttleByRaf } from '../system';
...
export const Affix = forwardRef<HTMLDivElement, AffixProps>((props, ref) => {
  const { offsetTop = 0, offsetBottom, target = () => window, targetContainer, onChange, children, ...rest } = props;
  ...
  useEffect(() => {
    updatePosition();
  }, [offsetTop, offsetBottom, target, targetContainer, updatePosition, measureWrapperInfo, measureAffixInfo]);
  ...
  useIsomorphicLayoutEffect(() => {
    if (status !== AffixStatus.START || !targetRef.current || !wrapperRef.current) {
      return;
    }
  ...
  return (
    <div ref={setWrapperRefs} css={applyBoxStyle(props)} {...deleteCssProps(rest)}>
      {lastIsFixed.current && <div css={applySize(size.width, size.height)}></div>}
      <div
