...
export function applyVerticalLineStyle(status: StepsStatus, type: StepsType, lineless?: boolean): SerializedStyles {
  let c = css``;
  switch (status) {
    case 'wait':
      c = css`
...
export function applyVerticalDescriptionStyle(type?: StepsType): SerializedStyles {
  return css`
    margin-right: 16px;
