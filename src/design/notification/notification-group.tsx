...
import { applyBoxStyle, BoxProps } from '../theme';

export const NotificationGroup: FC<BoxProps> = (props) => {
  const [topLeftNotificationList, setTopLeftNotificationList] = useState<NotificationProps[]>([]);
  const [topRightNotificationList, setTopRightNotificationList] = useState<NotificationProps[]>([]);
  const [bottomLeftNotificationList, setBottomLeftNotificationList] = useState<NotificationProps[]>([]);
  const [bottomRightNotificationList, setBottomRightNotificationList] = useState<NotificationProps[]>([]);
  ...
  return (
    <>
      <div css={[applyNotificationContainerStyle('topLeft'), applyBoxStyle(props)]}>
        <AnimatePresence>{topLeftNotification}</AnimatePresence>
      </div>
      <div css={[applyNotificationContainerStyle('topRight'), applyBoxStyle(props)]}>
        <AnimatePresence>{topRightNotification}</AnimatePresence>
      </div>
      <div css={[applyNotificationContainerStyle('bottomLeft'), applyBoxStyle(props)]}>
        <AnimatePresence>{bottomLeftNotification}</AnimatePresence>
      </div>
      <div css={[applyNotificationContainerStyle('bottomRight'), applyBoxStyle(props)]}>
