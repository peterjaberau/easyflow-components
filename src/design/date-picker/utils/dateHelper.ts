...
import { isDayjs, isArray, getDayjsValue, dayjsPro, getNow } from '../../system';
...
export function getLocaleDayjsValue(date: Dayjs | undefined, dayjsLocale: string): Dayjs | undefined {
...
export function getAllDaysByTime(dayStartOfWeek: number, isWeek: boolean, time: Dayjs) {
  const current = getTimeObj(time);

  const flatRows = new Array(ONE_PAGE_WITH_DAYS).fill('');
  const startIndex =
    current.day - dayStartOfWeek < 0 ? 7 + (current.day - dayStartOfWeek) : current.day - dayStartOfWeek;
  flatRows[startIndex] = {
    ...current,
  };
  // pre
...
export function getRangeDefaultValue(
  ...
  if (isHalfAvailable && (!rangeValue || (rangeValue && !(rangeValue as Dayjs[])[nextFocusedInputIndex]))) {
    const nv = [];
