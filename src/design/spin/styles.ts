...
export function applyTipsStyle(colorScheme: SpinColorScheme, size: SpinSize): SerializedStyles {
  return css`
...
export function applySizeCss(size: SpinSize, loading: boolean, colorScheme: SpinColorScheme): SerializedStyles {
  let sizeCss;
  switch (size) {
    case 'small':
      sizeCss = css`
...
export function applySpinContainerCss(loading: boolean): SerializedStyles | undefined {
  if (loading) {
    let background;
    if (loading) {
      background = css`
