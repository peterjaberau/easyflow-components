...
export const getUploadListContainerStyle = (type = 'text') => {
...
export const getTextItemContainerStyle = (listType = 'text') => {
  const paddingStyle =
    listType === 'text'
      ? css`
          padding: 7px 12px;
...
export const getTextItemContentContainerStyle = (listType = 'text') => {
  return css`
    display: flex;
    flex-wrap: nowrap;
    overflow: hidden;
...
export const getTextItemNameStyle = (status = 'init') => {
  let errorStyle = css``;
  if (status === 'error') {
    errorStyle = css`
