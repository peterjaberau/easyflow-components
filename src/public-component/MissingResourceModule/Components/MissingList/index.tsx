...
import { cellStyle, columnContainerStyle, missingListContainerStyle } from './style';
import React from 'react';

const MissingList: FC<MissingListProps> = (props) => {
  const { handleCancelModal } = props;
  const { actionIDMapAction, missingActionsMap, resourceIDMapResource, agentIDMapAgent, updateActions } =
  ...
  const handleChangePlaceInfo = (index: string, resourceID: string, actionIDs: string[]) => {
    ...
    <>
      <div css={missingListContainerStyle}>
        <header css={columnContainerStyle}>
          <p css={cellStyle('128px')}>{t('editor.action.panel.label.missing_resource.affected_actions')}</p>
          <p css={cellStyle('104px')}>{t('editor.action.panel.label.missing_resource.type')}</p>
          <p css={cellStyle('224px')}>{t('editor.action.panel.label.missing_resource.replacement')}</p>
          <p css={cellStyle('96px')}>{t('editor.action.panel.label.missing_resource.status')}</p>
          <p css={cellStyle('96px')}>{t('editor.action.panel.label.missing_resource.tutorial')}</p>
        </header>
