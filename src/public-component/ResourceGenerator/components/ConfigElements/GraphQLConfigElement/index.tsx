...
import { Divider, WarningCircleIcon } from '../../../../../design/react';
import { ResourceGeneratorContext } from '../../../provider';
import { urlValidate, validateNotEmpty } from '../../../utils';
import { ControlledElement } from '../../ControlledElement';
import { InputRecordEditor } from '../../InputRecordEditor';
import { BaseConfigElementProps } from '../interface';
import { container, errorIconStyle, errorMsgStyle, optionLabelStyle } from '../style';
...
const GraphQLConfigElement: FC<BaseConfigElementProps> = (props) => {
  ...
  const authType = watch('authentication', findResource?.content.authentication ?? GraphQLAuthValue.NONE);
  ...
  return (
    <>
      <div css={container}>
        ...
        />
        <Divider direction="horizontal" ml="24px" mr="24px" mt="16px" mb="8px" w="unset" />
        ...
        {authType === GraphQLAuthValue.BASIC && (
          <BasicAuthPanel control={control} auth={findResource?.content.authContent as GraphQLBasicAuth} />
        )}
        {authType === GraphQLAuthValue.BEARER && (
          <BearerAuthPanel control={control} auth={findResource?.content.authContent as GraphQLBearerAuth} />
        )}
        {authType === GraphQLAuthValue.APIKEY && (
          <APIKeyAuthPanel watch={watch} control={control} auth={findResource?.content.authContent as ApiKeyAuth} />
        )}
        <Divider direction="horizontal" ml="24px" mr="24px" mt="16px" mb="8px" w="unset" />
        <div css={optionLabelStyle}>{t('editor.action.resource.db.title.graphql_introspection')}</div>
        <ControlledElement
          title={t('editor.action.resource.db.label.introspection')}
          contentLabel={t('editor.action.resource.db.label.disable_introspection')}
