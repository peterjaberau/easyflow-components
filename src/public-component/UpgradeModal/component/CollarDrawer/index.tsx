...
import { SUBSCRIBE_PLAN, SUBSCRIPTION_CYCLE, USER_ROLE } from '../../../publicTypes';
import { TextLink } from '../../../TextLink';
import { getCurrentTeamInfo, getCurrentUserID } from '../../../UserData';
...
import { COLLAR_UNIT_BY_CYCLE, COLLAR_UNIT_PRICE } from '../../service/interface';
import { getSuccessRedirectWithParams, isSubscribeForDrawer, track } from '../../utils';
import { Calculator } from '../Calculator';
import { COLLAR_BUTTON_TEXT, COLLAR_MORE_TEXT, LEARN_MORE_LINK } from './constants';
...
export const CollarDrawer: FC<CollarDrawerProps> = (props) => {
  const { onCancel, visible, afterClose, onSuccessCallback, from, subCycle } = props;
  ...
  const isCancelSubscribe = currentTeamInfo?.colla?.plan === SUBSCRIBE_PLAN.COLLA_SUBSCRIBE_CANCELED;

  const teamQuantity = isSubScribe ? currentTeamInfo?.colla?.quantity ?? 0 : 0;
  const [currentQuantity, setCurrentQuantity] = useState<number>(teamQuantity + 1);
  const [cycle, setCycle] = useState<SUBSCRIPTION_CYCLE>(
    isSubScribe ? currentTeamInfo?.colla?.cycle ?? SUBSCRIPTION_CYCLE.MONTHLY : subCycle ?? SUBSCRIPTION_CYCLE.MONTHLY,
  );

  const currentCollarType = useRef<COLLAR_TYPE>(getCurrentCollarType(teamQuantity, currentQuantity, isCancelSubscribe));

  const disabledSubscribe =
    (currentQuantity === teamQuantity && cycle === currentTeamInfo?.colla?.cycle) ||
    (currentQuantity === 0 && cycle !== currentTeamInfo?.colla?.cycle);

  const changeNum = Math.abs(teamQuantity - currentQuantity);

  const [description, setDescription] = useState(getDescription(currentCollarType.current));
  ...
  const unitCollaByCycle = COLLAR_UNIT_BY_CYCLE[cycle ?? SUBSCRIPTION_CYCLE.MONTHLY];

  const calculatorNum = currentTeamInfo?.colla?.cycle === cycle ? changeNum : currentQuantity;
    ...
    try {
      switch (currentCollarType.current) {
        case COLLAR_TYPE.CANCEL_SUBSCRIPTION:
          await cancelSubscribe(currentTeamInfo.id, SUBSCRIBE_PLAN.COLLA_SUBSCRIBE_PAID);
  ...
  }, [currentTeamInfo?.id, currentTeamInfo?.myRole, from, userID, visible, reportElement]);
      ...
      <div css={drawerContentStyle}>
        <div>
          <div css={titleContainerStyle}>
            <div css={titleStyle}>{t('billing.payment_sidebar.title.colla')}</div>
          ...
          <div css={accountsStyle}>
            <div css={subTotalStyle}>
              ...
              <div css={priceStyle}>
                <div css={priceTotalStyle}>${(unitPrice * currentQuantity).toFixed(2)}</div>
          ...
          {!hiddenCalculator && <Calculator changeNum={calculatorNum} unitCollaByCycle={unitCollaByCycle} />}
        </div>
