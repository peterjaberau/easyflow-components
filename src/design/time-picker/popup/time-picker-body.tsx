...
import { getDayjsValue, getNow, padStart, toLocal, dayjsPro } from '../../system';
...
import { getHoursShowList, getMinutesShowList, getSecondsShowList, isUse12Hours } from './utils';
import { footerBtnWrapperStyle, footerExtraWrapperStyle, timepickerPopupStyle } from './style';
...
export const TimePickerPopup: FC<TimePickerPopupProps> = (props) => {
  ...
  const getDefaultStr = useCallback(
      ...
      switch (type) {
        ...
        case 'minute':
          return typeof disabledMinutes === 'function'
            ? padStart(`${MINUTES.find((m) => disabledMinutes(selectedHour).indexOf(m) === -1) || 0}`, 2, '0')
            : padStart(`${MINUTES[0]}`, 2, '0');
        case 'second':
          return typeof disabledSeconds === 'function'
            ? padStart(
                `${SECONDS.find((s) => disabledSeconds(selectedHour, selectedMinute).indexOf(s) === -1) || 0}`,
                2,
                '0',
              )
    ...
    [HOURS, MINUTES, SECONDS, disabledHours, disabledMinutes, disabledSeconds, selectedHour, selectedMinute],
  );
  ...
  const onHandleSelect = (selectedValue?: number | string, unit?: string) => {
    ...
    const _valueShow =
      valueShow ||
      dayjsPro(`${getDefaultStr('hour')}:${getDefaultStr('minute')}:${getDefaultStr('second')}`, 'HH:mm:ss');
    ...
    if (unit === 'hour') {
      newValue = dayjsPro(`${selectedValue}:${minute}:${second} ${selectedAmpm}`, valueFormat, 'en');
    }
    if (unit === 'minute') {
      newValue = dayjsPro(`${hour}:${selectedValue}:${second} ${selectedAmpm}`, valueFormat, 'en');
    }
    if (unit === 'second') {
      newValue = dayjsPro(`${hour}:${minute}:${selectedValue} ${selectedAmpm}`, valueFormat, 'en');
    }
    if (unit === 'ampm') {
      newValue = dayjsPro(
        `${hour}:${minute}:${second} ${isUpperCase ? (selectedValue as string).toUpperCase() : selectedValue}`,
        valueFormat,
        'en',
      );
    }
  ...
  const _hideFooter =
    hideFooter || (disableConfirm && isRangePicker) || (!isRangePicker && disableConfirm && !showNowBtn);
  return (
    <>
