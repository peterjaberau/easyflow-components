...
import { disableInviteLink, enableInviteLink, getInviteLink, renewInviteLink } from '../service';
import {
...
export const InviteLinkPC: FC<InviteLinkProps> = (props) => {
  ...
  const [inviteUserRole, setInviteUserRole] = useMergeValue(defaultInviteUserRole, {
    defaultValue: defaultInviteUserRole,
  });

  const [allowInviteLink, setAllowInviteLink] = useMergeValue(defaultAllowInviteLink, {
  ...
  useEffect(() => {
    ...
    const getInviteLinkRequest = async () => {
      ...
      try {
        const data = await getInviteLink(teamID, inviteUserRole, redirectURL, window.customDomain, controller.signal);
  ...
  }, [currentUserRole, teamID, allowInviteLink, inviteUserRole, message, t, redirectURL]);

  const renewInviteLinkRequest = useCallback(
    async (teamID: string, userRole: USER_ROLE) => {
      setGetLinkLoading(true);
      try {
        const data = await renewInviteLink(teamID, redirectURL, userRole, window.customDomain);
  ...
  return isSmallThanTargetRole(USER_ROLE.ADMIN, currentUserRole, false) && !allowInviteLink ? null : (
    <div css={inviteLinkContainer}>
      {(allowInviteLink || (!allowInviteLink && isBiggerThanTargetRole(USER_ROLE.ADMIN, currentUserRole))) && (
        <div css={inviteLinkMenuContainer}>
          <div css={inviteLinkLabelStyle}>{t('user_management.modal.link.invite_title')}</div>
          {allowInviteLink && isBiggerThanTargetRole(USER_ROLE.ADMIN, currentUserRole) && (
            <Dropdown
              trigger="click"
              position="bottom-end"
              dropList={
      ...
      {allowInviteLink ? (
        <div css={inviteLinkCopyContainer}>
          <Input
            ...
            value={
              getLinkLoading ? <Skeleton text={{ rows: 1 }} opac={0.5} animation flexGrow="1" /> : currentInviteLink
            }
            suffix={
              <RoleSelector
                inline
                ...
                onClickItem={async (role) => {
                  if (isBiggerThanTargetRole(USER_ROLE.VIEWER, role, false) && defaultBalance === 0) {
                    upgradeModal({
                      modalType: 'upgrade',
                      from: 'invite_by_link',
                    });
          ...
          <Button
            ...
            onClick={() => {
              if (isBiggerThanTargetRole(USER_ROLE.VIEWER, inviteUserRole, false) && defaultBalance === 0) {
                upgradeModal({
                  modalType: 'upgrade',
                  from: 'invite_by_link',
                });
        ...
        isBiggerThanTargetRole(USER_ROLE.ADMIN, currentUserRole) && (
          <div css={closeInviteLinkContainerStyle}>
            <div css={secretLinkStyle}>{t('user_management.modal.link.description')}</div>
            <Button
              variant="text"
              size="small"
              loading={getLinkLoading}
