...
export const BasicFooterSection: FC<BasicFooterProps> = (props) => {
  const { showTime = false, disabled, onClickConfirmBtn, onSelectNow, showNowBtn, extra, mode, placeLeft } = props;
      ...
      {showTime ? (
        <div css={footerBtnWrapperStyle}>
          ...
          {mode === 'date' && showNowBtn && (
            <Button className="btn-now" size="medium" onClick={onSelectNow} colorScheme="grayBlue">
              {DATE_PICKER_LOCALE.nowText}
            </Button>
          )}
          {showTime && (
            <>
              <Button className="btn-confirm" size="medium" disabled={disabled} onClick={onClickConfirmBtn}>
