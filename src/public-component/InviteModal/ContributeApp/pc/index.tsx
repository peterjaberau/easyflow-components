...
import { Checkbox, Input, Modal, TextArea, getColor, useMessage } from '../../../../design/react';
...
import { contributeAppWithHashtags, updateAppConfig, updateAppContribute } from '../service';
import { blockCheckboxStyle, blockLabelStyle, blockRequireStyle, blockStyle } from './style';
...
export const ContributeAppPC: FC<ContributeAppProps> = (props) => {
  ...
  return (
    <Modal
      ...
      okText={props.productContributed ? t('contribute.update_modal.button') : t('contribute.first_time_modal.button')}
      onOk={handleSubmit(async (data) => {
        ...
        try {
          if (props.productContributed) {
            await updateAppContribute(props.teamID, props.productID, data.hashtags);
          } else {
            await contributeAppWithHashtags(props.teamID, props.productID, data.hashtags);
          }
      ...
      title={props.productContributed ? t('contribute.update_modal.title') : t('contribute.first_time_modal.title')}
    >
      <Controller
        name="appName"
        control={control}
      ...
      <Controller
        ...
        render={({ field }) => (
          <section css={blockStyle}>
            <label css={blockLabelStyle}>{t('new_dashboard.app_setting.description')}</label>
            <TextArea
              {...field}
              showWordLimit
              maxLength={180}
              ...
              placeholder={t('new_dashboard.app_setting.placeholder.description')}
            />
          </section>
        )}
      />
        ...
        <Controller
          ...
          render={({ field }) => (
            <div css={blockCheckboxStyle}>
              <Checkbox {...field} value="" checked={field.value} colorScheme="techPurple">
