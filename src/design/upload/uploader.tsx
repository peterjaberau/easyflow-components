...
import { UploaderProps, UploadItem, STATUS, RequestOptions, UploaderInstance } from './interface';
import uploadRequestSender from './request';
import { AnimatePresence, motion } from 'framer-motion';
import TriggerNode from './trigger-node';
import { isFunction, isFile, isNumber } from '../system';
...
const Uploader = forwardRef<UploaderInstance, UploaderProps>((props, ref) => {
  ...
  const handleUploadedFiles = useCallback(
    (files: File[]) => {
      if (isNumber(limit) && limit < (fileList ? fileList.length : 0) + files.length) {
    ...
    [limit, fileList, onExceedLimit, accept, directory, beforeUpload, uploadSingleFile],
  );
  ...
  return (
    <>
      ...
      {tip && listType !== 'picture-card' && !drag ? <div key="trigger-tip">{tip}</div> : null}
    </>
  );
});
