...
export const ResourceGenerator: FC<ResourceGeneratorProps> = (props) => {
  ...
  const [currentStep, setCurrentStep] = useState<ResourceCreatorPage>(defaultConfig?.defaultStep ?? 'select');
  ...
  const onCancel = () => {
    const element = currentStep === 'createResource' ? 'resource_configure_close' : 'resource_type_modal_close';
  ...
  useEffect(() => {
    if (visible) {
      const element = currentStep === 'createResource' ? 'resource_configure_modal' : 'resource_type_modal';
      track?.(
