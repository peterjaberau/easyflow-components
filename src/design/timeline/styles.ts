...
import { TimelineDirection, TimelineItemLineType, TimelineMode } from './interface';
import { ReactNode } from 'react';

export function applyWrapStyle(direction: TimelineDirection, mode: TimelineMode): SerializedStyles {
  return css`
    display: inline-flex;
...
export function applyItemStyle(direction: TimelineDirection, mode: string, dot?: ReactNode): SerializedStyles {
...
export function baseLineStyle(lineColor?: string): SerializedStyles {
  const color = lineColor ? lineColor : globalColor(`--${illaPrefix}-grayBlue-08`);
  return css`
    border-color: ${color};
  `;
}

export function dotCommonStyle(dotColor?: string, dotType?: string): SerializedStyles {
  const dotFillColor = dotColor ? dotColor : globalColor(`--${illaPrefix}-blue-03`);
  let dotFillStyle;
  if (dotType === 'hollow') {
    dotFillStyle = css`
...
export function applyDotItemStyle(direction: TimelineDirection, mode: string): SerializedStyles {
...
export function applyLineStyle(
  ...
  const dotFillColor = dotColor ? dotColor : globalColor(`--${illaPrefix}-grayBlue-08`);
  if (direction === 'horizontal') {
    return css`
...
export function applyDotWrapperStyle(direction: TimelineDirection): SerializedStyles {
  if (direction === 'horizontal') {
    return css`
      ${flexCenter};
      width: 24px;
