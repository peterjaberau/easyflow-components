...
import { ButtonColorScheme, ButtonShape, ButtonSize, ButtonVariant } from './interface';
import { getColor, getSpecialThemeColor, hasGlobalColor, illaPrefix, zIndex } from '../theme';

// button
export function applyTagContainer(fullWidth?: boolean, fullHeight?: boolean): SerializedStyles {
  return css`
    ${fullWidth
      ? css`
          width: 100%;
      ...
      switch (variant) {
        ...
        case 'light': {
          if (colorScheme === 'white') {
            ...
            return [getColor(colorScheme, '08'), getSpecialThemeColor(colorScheme)];
          }
        }
        case 'dashed':
        case 'outline':
          if (colorScheme == 'white') {
            ...
            return [getSpecialThemeColor(colorScheme), getSpecialThemeColor(colorScheme)];
          } else {
        ...
        case 'text':
          if (colorScheme == 'white') {
            ...
            return [getColor('transparent', '01'), getSpecialThemeColor(colorScheme)];
          }
      }
    case State.HOVER:
      switch (variant) {
        ...
        case 'light': {
          if (colorScheme == 'white') {
            ...
            return [getColor(colorScheme, '07'), getSpecialThemeColor(colorScheme)];
          }
        }
        case 'dashed':
        case 'outline':
      ...
      switch (variant) {
        ...
        case 'light': {
          if (colorScheme == 'white') {
            ...
            return [getColor(colorScheme, '06'), getSpecialThemeColor(colorScheme)];
          }
        }
        case 'dashed':
        case 'outline':
        ...
        case 'text':
          if (colorScheme == 'white') {
            ...
            return [getColor('grayBlue', '08'), getSpecialThemeColor(colorScheme)];
          }
      }
    case State.DISABLE:
      switch (variant) {
        case 'fill':
          if (colorScheme == 'white') {
            ...
            return [
              getColor(colorScheme, hasGlobalColor(`--${illaPrefix}-${colorScheme}-07`) ? '07' : '05'),
              getColor('white', '01'),
            ];
          } else {
...
export function applyBg(variant: ButtonVariant, colorScheme: ButtonColorScheme): SerializedStyles {
  switch (variant) {
    case 'text':
      return css`
        &:hover {
...
export function applyElementColor(variant: ButtonVariant, colorScheme: ButtonColorScheme): SerializedStyles {
  switch (variant) {
    case 'text':
      return css`
...
export function applyShape(shape: ButtonShape, attached: boolean, first: boolean, last: boolean): SerializedStyles {
  switch (shape) {
    case 'square':
      if (attached) {
        if (first) {
...
export function applyPaddingStyle(size: ButtonSize, variant: ButtonVariant): SerializedStyles {
  switch (size) {
    case 'small':
      return css`
        padding: ${variant == 'outline' || variant == 'dashed' ? '1px 7px' : '2px 8px'};
        min-height: 24px;
      `;
    case 'medium':
      return css`
...
export function applyWithoutTextSize(size: ButtonSize, fullWidth?: boolean, fullHeight?: boolean): SerializedStyles {
  switch (size) {
    case 'small':
      return css`
        justify-content: center;
...
export function applyLeftIconStyle(size: ButtonSize, hasChildren?: boolean): SerializedStyles {
  switch (size) {
    case 'small':
      return css`
        display: inline-flex;
...
export function applyRightIconStyle(size: ButtonSize, hasChildren?: boolean): SerializedStyles {
  switch (size) {
    case 'small':
      return css`
        display: inline-flex;
