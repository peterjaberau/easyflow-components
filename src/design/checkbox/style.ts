...
export function applyCheckboxSize(checked?: boolean, colorScheme: CheckboxColorScheme = 'blue') {
  let checkedCss = css();
  if (checked) {
    checkedCss = css`
...
export function applyCheckboxContainerHorizontal(spacing: string | number): SerializedStyles {
...
export function applyCheckboxContainerVertical(spacing: string | number): SerializedStyles {
  const currentSpacing = typeof spacing === 'string' ? spacing : `${spacing}px`;
