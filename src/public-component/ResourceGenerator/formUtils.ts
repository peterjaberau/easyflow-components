import { isILLAAPiError } from '../ILLANet';
import { DATABASE_INDEX, DEFAULT_NAME, generateGraphQLAuthContent, generateSSLConfig } from '../publicConfig';
import { AccessType, Resource, ResourceContent, ResourceType, RestApiAuth } from '../publicTypes';
import { ILLAPublicStorage } from '../utils';
import { FieldValues, UseFormHandleSubmit } from 'react-hook-form';
import { getI18n } from 'react-i18next';
import { createMessage } from '../../design/react';
import { IActionTestConnectionRequestData, fetchActionTestConnection } from './service';
import { getOAuthAccessToken, redirectToGoogleOAuth } from './service/oAuth';
import { requestCreateResource, requestUpdateResource } from './service/resource';
...
function getActionContentByType(data: FieldValues, type: ResourceType) {
  switch (type) {
      ...
      return {
        ...
        configContent:
          data.configType === 'gui'
            ? {
                host: data.host.trim(),
                port: data.connectionFormat === 'standard' ? data.port.toString() : '',
    ...
    case 'couchdb': {
      const { resourceName: _couchDBResName, host, ...otherCouchDBParams } = data;
    ...
    case 'googlesheets':
      const status = data.authentication === 'oauth2' && ILLAPublicStorage.getLocalStorage('oAuthStatus');
      let oAuthOpts = {};
      if (status) {
...
export const generateRestAPIAuthContent = (data: { [p: string]: any }): RestApiAuth => {
...
export function onActionConfigElementSubmit(
  ...
  finishedHandler: (resource: Resource<ResourceContent>, isUpdate: boolean) => void,
) {
  const isUpdate = resourceID != undefined;
  const googleAuthMethod = async (resourceID: string, accessType: AccessType) => {
  ...
  return handleSubmit(async (data: FieldValues) => {
    ...
    try {
      if (isUpdate) {
        const response = await requestUpdateResource(teamID, resourceID, requestData);
      ...
      if (isILLAAPiError(e)) {
        message.error({
          content: e.data.errorMessage || getI18n().t('dashboard.resource.save_fail'),
        });
      } else {
        message.error({
