...
import { ILLAMixpanel, ILLA_MIXPANEL_EVENT_TYPE, ILLA_MIXPANEL_PUBLIC_PAGE_NAME } from '../../../MixpanelUtils';
...
import { Button, Input, Password } from '../../../../design/react';
...
export const MobileRegister: FC<RegisterProps> = (props) => {
  const { lockedEmail, onSubmit, errorMsg, loading, showCountDown, hideOAuth, onCountDownChange, sendEmail } = props;
  ...
  return (
    <form css={formStyle} autoComplete="off" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div css={formItemStyle}>
        ...
        />
        {formState?.errors.nickname && <div css={errorMsgStyle}>{formState?.errors.nickname.message}</div>}
      </div>
      <div css={formItemStyle}>
        <Controller
          name="email"
        ...
        {(formState?.errors.email || errorMsg.email) && (
          <div css={errorMsgStyle}>{formState?.errors.email?.message || errorMsg.email}</div>
        )}
      </div>
      {isCloudVersion && (
        <div css={formItemStyle}>
          ...
          {(formState?.errors.verificationCode || errorMsg.verificationCode) && (
            <div css={errorMsgStyle}>{formState?.errors.verificationCode?.message || errorMsg.verificationCode}</div>
          )}
        </div>
      )}
      <div css={formItemStyle}>
        ...
        />
        {formState?.errors.password && <div css={errorMsgStyle}>{formState?.errors.password.message}</div>}
      </div>
      ...
      {isCloudVersion && !hideOAuth && (
        <div css={[oAuthButtonGroupStyle]}>
          <OAuthButton icon={<GithubIcon css={oAuthIconStyle} />} type="github" isMobile landing="signup" />
        </div>
      )}
    </form>
  );
