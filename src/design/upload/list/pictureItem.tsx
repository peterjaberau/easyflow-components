...
import { isFunction, isObject } from '../../system';
import UploadProgress from './uploadProgress';
import { EyeOnIcon, UploadIcon, DeleteIcon, ImageDefaultIcon } from '../../icon';
import {
...
const PictureItem = (props: UploadListItemProps) => {
  const { disabled, file, showUploadList, onRemove, onPreview, onReupload } = props;
  const { status } = file;
  ...
  const imageDom = url && isImageUrl(file) ? <img src={url} alt={file.name} /> : <Icon />;
  ...
  const reuploadIcon =
    file.status === STATUS.FAIL && (actionIcons.reuploadIcon !== null || onReupload) ? (
      <span
