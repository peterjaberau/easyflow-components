...
export const TabPane: FC<TabPaneProps> = (props) => {
  const { title, disabled = false, closable, tabsItemAfter, tabsItemColorScheme, tabsItemActiveColorScheme } = props;
  const {
    selectedKey,
    size = 'medium',
    tabPosition = 'top',
  ...
  return (
    ...
    >
      <div css={panelItemStyle(tabPosition)}>
        <div css={panelTitleStyle(size, tabPosition, variant, disabled, isSelectKey)}>
          <span css={titleStyle(disabled, isSelectKey, colorScheme, tabsItemColorScheme, tabsItemActiveColorScheme)}>
            {title}
          </span>
        ...
        {variant === 'line' && isSelectKey ? (
          <motion.div
            css={lineStyle(colorScheme, tabPosition, disabled, tabsItemActiveColorScheme)}
            layoutId="underline"
          />
        ) : null}
      </div>
      {tabsItemAfter && <div css={tabsItemAfterStyle(tabPosition)}>{tabsItemAfter}</div>}
    </div>
  );
};
