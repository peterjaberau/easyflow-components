...
import { getUploadListContainerStyle, listItemStyle, uploadListItemStyle } from '../style';

const UploadList = (props: UploadListProps) => {
  const { listType, fileList = [], renderUploadList, renderUploadItem, onPreview, ...rest } = props;

  if (isFunction(renderUploadList)) {
    return <div css={getUploadListContainerStyle(listType)}>{renderUploadList(fileList, rest)}</div>;
  }
    ...
    let originNode: ReactNode =
      listType === 'picture-card' ? (
        <div css={listItemStyle}>
          <PictureItem {...props} onPreview={handlePictureItemPreview} file={file} />
        </div>
      ) : (
