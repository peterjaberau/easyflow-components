...
import { Button, CloseIcon, Divider, Drawer, InputNumber, useMessage } from '../../../../design/react';
...
export const StorageDrawer: FC<StorageDrawerProps> = (props) => {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      const res = handleCollaPurchaseError(error, CollarModalType.STORAGE, 'billing_storage_balance_manage');
      if (res) return;
      if (driveVolume?.plan && isSubscribeForDrawer(driveVolume?.plan)) {
        message.error({
      ...
      <div css={drawerContentStyle}>
        <div>
          <div css={drawerPaddingStyle}>
            ...
            <div css={titleStyle}>{t('billing.payment_sidebar.title.manage_storage')}</div>
          ...
          <div css={drawerPaddingStyle}>
            <div css={subTotalStyle}>
              ...
              <div css={priceStyle}>
                <div css={priceTotalStyle}>{UNIT_COLLA_BY_STORAGE * quantity}K Colla</div>
                <div css={priceTotalLabelStyle}>
                  {t('billing.payment_sidebar.price_cal.next_period_monthly_remove_storage', {
