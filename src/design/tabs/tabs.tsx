...
import { forwardRef, ReactElement, useState, useMemo, Children, useCallback, useEffect, useRef } from 'react';
import { TabPaneProps, TabsProps } from './interface';
import { TabPane } from './tab-pane';
import { TabsProvider } from './context/context';
import { iconStyle, nextIconStyle, tabsContainerStyle, tabsStyle } from './style';
import { UpIcon, DownIcon, PreviousIcon } from '../icon';
...
export const Tabs = forwardRef<HTMLDivElement, TabsProps>((props, ref) => {
  ...
  const [selectedKey, setSelectedKey] = useState(defaultActiveKey ?? firstTabKey);
  const layoutId = v4();
  ...
  const handleGoNext = () => {
    if (containerRef.current && panelRef.current) {
      if (
        -(translate - containerRef.current.clientWidth / 2) + containerRef.current.clientWidth + SCROLL_ICON_WIDTH >=
        panelRef.current.scrollWidth
      ) {
        setTranslate(-(panelRef.current.scrollWidth + SCROLL_ICON_WIDTH - containerRef.current.clientWidth));
      } else {
  ...
  const handleGoPrevious = () => {
    if (containerRef.current && panelRef.current) {
      if (translate + containerRef.current.clientWidth / 2 >= SCROLL_ICON_WIDTH) {
  ...
  const handleShowScrollIcon = useCallback((width?: number) => {
    if (containerRef.current && panelRef.current) {
      if (width) {
        ...
        if (containerRef.current.clientWidth + SCROLL_ICON_WIDTH < panelRef.current.scrollWidth) {
          setShowScrollIcon(true);
        } else {
          setShowScrollIcon(false);
        }
  ...
  return (
    <LayoutGroup id={layoutId}>
      ...
      >
        <div css={tabsContainerStyle(tabPosition, align, variant, withoutBorderLine)} ref={containerRef}>
          {prefix}
          {!isHorizontalLayout(tabPosition) && showScrollIcon && (
            <span css={iconStyle('previous')} onClick={handleGoPrevious}>
              {isHorizontalLayout(tabPosition) ? <UpIcon size="12" /> : <PreviousIcon size="12" />}
            </span>
          )}
          <div ref={panelRef} css={tabsStyle(tabPosition, tabBarSpacing, variant, translate, align, withoutBorderLine)}>
            {curTabItems}
          </div>
          {!isHorizontalLayout(tabPosition) && showScrollIcon && (
