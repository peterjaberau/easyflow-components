...
import { CloseIcon, ErrorCircleIcon, InfoCircleIcon, LoadingIcon, WarningCircleIcon, SuccessCircleIcon } from '../icon';
...
export const Message = forwardRef<HTMLDivElement, MessageProps>((props, ref) => {
  const { type, closable, showIcon = true, duration, id = '', position, icon, content, onClose } = props;
  ...
  return (
    ...
    >
      {normalIcon}
      <span css={applyMessageTextStyle(showIcon && type !== 'normal')}>{content}</span>
      {closable && (
        <CloseIcon
          fs="8px"
          ml="15px"
