...
const getActiveKeys = (keys: string | string[], accordion?: boolean): string[] => {
...
export const Collapse = forwardRef<HTMLDivElement, CollapseProps>((props, ref) => {
  ...
  const [activeKeys, setActiveKeys] = useMergeValue<string[]>([], {
    defaultValue: defaultActiveKey ? getActiveKeys(defaultActiveKey, accordion) : undefined,
  ...
  return (
    ...
    >
      <div ref={ref} css={[applyCollapseStyle(bordered), applyBoxStyle(props)]} {...deleteCssProps(otherProps)}>
        {children}
      </div>
    </CollapseContext.Provider>
  );
