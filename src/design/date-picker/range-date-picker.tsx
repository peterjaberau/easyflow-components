...
import { FC, useEffect, useMemo, useRef, useState, MouseEvent, ChangeEvent, forwardRef } from 'react';
import React from 'react';
import { GetHeaderOperationsFun, DatePickerModeType, RangeDatePickerProps } from './interface';
...
import { getAvailableDayjsLength, getLocaleDayjsValue, getRangeDefaultValue } from './utils/dateHelper';
import { useUpdate } from './utils/hooks';
import { DateInputRangeHandler } from './input/interface';
import { CalendarIcon, TimeIcon } from '../icon';
...
import { applyBoxStyle } from '../theme';

export const RangeDatePicker = forwardRef<HTMLDivElement, RangeDatePickerProps>((props, ref) => {
  const {
    allowClear = true,
    placeholder,
    disabled,
  ...
  const [focusedInputIndex, setFocusedInputIndex] = useState<number>(isHalfAvailable ? availableInputIndex : 0);
  ...
  const [panelModes, setPanelModes] = useState<DatePickerModeType[]>([mode, mode]);
  ...
  const propsValueDayjs = getDayjsValue(propsValue as Dayjs[], format, utcOffset, timezone) as Dayjs[];
  ...
  const defaultPageShowDates = mergedValue ||
    (getDayjsValue(defaultPickerValue as Dayjs[], format) as Dayjs[]) || [now, now];

  function isValidDayjsArray(sv: unknown) {
    return sv && Array.isArray(sv) && sv.length === 2 && isDayjs(sv[0]) && isDayjs(sv[1]);
  }

  function isSamePanel(innerValue: Dayjs[] | undefined, pickerMode: DatePickerModeType) {
    if (innerValue && innerValue.length === 2 && isValidDayjsArray(innerValue)) {
      return (
        ((pickerMode === 'date' || pickerMode === 'week') && innerValue[0].isSame(innerValue[1], 'month')) ||
        ((pickerMode === 'month' || pickerMode === 'quarter') && innerValue[0].isSame(innerValue[1], 'year')) ||
        (pickerMode === 'year' && Math.floor(innerValue[0].year() / 10) === Math.floor(innerValue[1].year() / 10))
      );
    }
  }
  ...
  function getPageShowDatesByValue(
    ...
    switch (pickerMode) {
      case 'date':
      case 'week':
        return prev ? [value, value.add(1, 'month')] : [value.subtract(1, 'month'), value];
      case 'month':
      case 'quarter':
        return prev ? [value, value.add(1, 'year')] : [value.subtract(1, 'year'), value];
      case 'year':
        return prev ? [value, value.add(10, 'year')] : [value.subtract(10, 'year'), value];
      default:
        return [];
    }
  }
  ...
  const mergedPageShowDate =
    getShowDatesFromFocused(getDayjsValue(pickerValue as Dayjs[], format, utcOffset, timezone) as Dayjs[]) ||
    pageShowDates;
  ...
  useUpdate(() => {
    if (Array.isArray(value) && (previousUtcOffset !== utcOffset || timezone !== previousTimezone)) {
  ...
  function handlePickerValueChange(v: Dayjs[]) {
    if (!isSamePanel([v[0], pageShowDates?.[0] as Dayjs], mode)) {
      onPickerValueChange && onPickerValueChange(Array.isArray(v) ? v.map((v) => v && v.format(format)) : undefined, v);
    }
  }

  function setFixedPageShowDates(innerValue: Dayjs[], index = focusedInputIndex) {
  ...
  function onHandleChange(newValue: Dayjs[] | undefined) {
    if (isDayjsArrayChange(mergedValue, newValue)) {
      const localValue = Array.isArray(newValue)
        ? newValue.map((v) => getLocaleDayjsValue(toLocal(v, utcOffset, timezone), 'en-us'))
        : undefined;
      onChange &&
        onChange(
          Array.isArray(localValue) ? (localValue.map((v) => v && v.format(format)) as string[]) : undefined,
          localValue as Dayjs[],
        );
    }
  }
  ...
  function isDisabledDate(date: Dayjs): boolean {
    const selectedDisabled = typeof selectedDisabledDate === 'function' ? selectedDisabledDate(date) : false;
  ...
  function isValid(time?: string): boolean {
    return isValidTimeString(time, format) && !isDisabledDate(getDayjsValue(time, format) as Dayjs);
  }
  ...
  function onClickConfirmBtn() {
    onConfirmValue();
    const localePanelValue = panelValue.map((v) => getLocaleDayjsValue(v, 'en-us'));
    onOk && onOk(localePanelValue.map((v) => v && v.format(format)) as string[], localePanelValue as Dayjs[]);
  }
  ...
  function onSelectPanel(_: string | undefined, date: Dayjs | undefined) {
    ...
    const newValueShow = (resetRange && selectedLength === 2 && !isHalfAvailable ? [] : [...panelValue]) as Dayjs[];
    ...
    const focusedIndex = customTriggerElement
      ? selectedLength === 0 || selectedLength === 2
        ...
        : 1
      : focusedInputIndex;
    const newDate = (showTime ? getValueWithTime(date as Dayjs, timeValues[focusedIndex]) : date) as Dayjs;
  ...
  function onSelectValueShow(newValueShow: Dayjs[]) {
    ...
    const zoneValues = sortedValues?.map((v) => getLocaleDayjsValue(toLocal(v, utcOffset, timezone), 'en-us'));
    onSelect &&
      onSelect(zoneValues?.map((v) => v && v.format(format)) as string[], zoneValues as Dayjs[], {
  ...
  const suffixIcon = inputSuffix === null ? null : inputSuffix || (showTime ? <TimeIcon /> : <CalendarIcon />);

  const triggerDisabled = Array.isArray(disabled) ? disabled[0] && disabled[1] : disabled;

  const shouldShowFooter = (showTime && panelModes[0] === 'date' && panelModes[1] === 'date') || extra;
