...
import { useEffect, useRef, useState, MouseEvent, ChangeEvent, forwardRef, useCallback } from 'react';
import { getDayjsValue, isDayjs, isDayjsChange, toLocal, toTimezone, usePrevious, isValidTimeString } from '../system';
...
export const TimePicker = forwardRef<HTMLDivElement, TimePickerProps>((props, ref) => {
  ...
  const onHandleChange = (vs?: Dayjs) => {
    if (isDayjs(vs) && isDayjsChange(mergedValue as Dayjs, vs)) {
      onChange && onChange(toLocal(vs, utcOffset, timezone).format(format), toLocal(vs, utcOffset, timezone));
    }
  };
