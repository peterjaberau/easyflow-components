...
export const CardMeta = forwardRef<HTMLDivElement, CardMetaProps>((props, ref) => {
  const {
    title,
    size = 'medium',
    description,
  ...
  return (
    <div ref={ref} css={[applyMetaContainer(hoverable), applyBoxStyle(props)]} {...deleteCssProps(restProps)}>
      {cover}
      <div css={[applyCardMetaBody(size, bordered, cover)]}>
        {title || description ? (
          <>
            {title && <div css={applyCardMetaTitle}>{title}</div>}
            {description && <div css={applyCardMetaDescription}>{description}</div>}
          </>
        ) : null}
