import { HTTP_REQUEST_PUBLIC_BASE_URL } from '../../ILLANet';
import { CurrentUserInfo, TeamInfo } from '../../publicTypes';
  ...
  endpoints: (builder) => ({
    ...
    >({
      async queryFn(teamIdentifier, _queryAPI, _extraOptions, fetchWithBQ) {
        ...
        const currentTeamID = teamInfos.find((info) => info.identifier === teamIdentifier)?.id;
        return {
          data: {
            user: userInfo,
            teams: teamInfos,
