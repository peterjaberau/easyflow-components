...
import { globalColor, getSpecialThemeColor, illaPrefix, getColor } from '../theme';
...
export function applyFontColor(colorScheme: TypographyColorScheme): SerializedStyles {
  const color = colorSchemes.includes(colorScheme) ? getSpecialThemeColor(colorScheme) : colorScheme;
  return css`
    color: ${color};
  `;
}
...
export function applyFontContentStyle(
  ...
  if (typeof mark == 'string') {
    const bgColor = colorSchemes.includes(mark) ? globalColor(`--${illaPrefix}-${mark}-07`) : mark;
    finalCss = css`
      ${finalCss};
