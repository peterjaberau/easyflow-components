...
import { RestApiResourceInit } from '../../../../publicConfig';
import { Resource, RestApiAuth, RestApiAuthType, RestApiResource, VerifyMode } from '../../../../publicTypes';
import { FC, useContext } from 'react';
import { Controller, useFormContext } from 'react-hook-form';
import { useTranslation } from 'react-i18next';
import { Alert, Divider } from '../../../../../design/react';
...
const RestApiConfigElement: FC<BaseConfigElementProps> = (props) => {
  ...
  const findResource = getResourceByID(resourceID) as Resource<RestApiResource<RestApiAuth>>;

  const content = findResource?.content ?? RestApiResourceInit;

  const showCertificates = (watch('baseUrl', content.baseUrl) ?? '').startsWith('https://');
  ...
  const SubPanelComponent = RestApiAuthTypeComponentMap[authType as keyof typeof RestApiAuthTypeComponentMap];
  ...
  const AuthenticationOptions: {
    ...
    {
      label: t('editor.action.resource.restapi.option.authentication.basic_auth'),
      value: 'basic',
    },
    {
  ...
  const VerificationModeOptions: {
    ...
    {
      label: t('editor.action.form.option.restapi.verification_mode.full_verification'),
      value: 'verify-full',
    },
    {
      label: t('editor.action.form.option.restapi.verification_mode.verify_ca_certificat'),
      value: 'verify-ca',
    },
    {
      label: t('editor.action.form.option.restapi.verification_mode.skip_ca_certificate_'),
      value: 'skip',
    },
  ];
    ...
    <>
      <div css={container}>
        ...
        <ControlledElement
          ...
          placeholders={[t('editor.action.resource.restapi.placeholder.base_url')]}
        />
        <Controller
          control={control}
        ...
        {showCertificatesConfig && (
          <>
            <ControlledElement
              ...
              name="caCert"
              placeholders={[t('editor.action.resource.db.placeholder.certificate')]}
            />
            <ControlledElement
              controlledType={['textarea']}
              ...
              <ControlledElement
                ...
                tips={
                  <Alert
                    ...
                    content={t('editor.action.form.tips.restapi.verification_mode.skip_ca_certificate')}
                  />
                }
              />
            )}
        ...
        {SubPanelComponent && <SubPanelComponent control={control} auth={content.authContent} />}
      </div>
    </>
  );
};
