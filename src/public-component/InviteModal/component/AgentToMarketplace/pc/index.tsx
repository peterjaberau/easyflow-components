...
import { BindIcon, Button, PenIcon, Switch, getColor, useMergeValue, useMessage } from '../../../../../design/react';
...
export const AgentToMarketplacePC: FC<AgentToMarketplaceProps> = (props) => {
  ...
  const [agentContributed, setAgentContributed] = useMergeValue(defaultAgentContributed, {
  ...
  return (
    <>
      <div css={publicContainerStyle}>
        {(isBiggerThanTargetRole(USER_ROLE.VIEWER, userRoleForThisAgent, false) || agentContributed) && (
        ...
        {agentContributed ? (
          ...
          <div css={contributingDocStyle}>{t('user_management.modal.contribute.desc')}</div>
        )}
        {agentContributed && <ShareBlockPC onShare={onShare} title={title} shareUrl={getAgentPublicLink(agentID)} />}
      </div>
