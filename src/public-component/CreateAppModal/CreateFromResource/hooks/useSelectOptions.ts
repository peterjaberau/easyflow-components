...
export const useSelectOptions = () => {
  ...
  const getSelectValue = useCallback(
    ...
    [ARRAY_INPUT_OPTIONS, BOOLEAN_OPTIONS, DATE_OPTIONS, NUMBER_INPUT_OPTIONS, STRING_INPUT_OPTIONS],
  );
  ...
  const getInputType = useCallback(
    (resourceType: RESOURCE_TYPE, type: string) => {
      ...
      return {
        ...
        label: selectOptions?.options.find(({ value }) => value === selectOptions?.defaultValue)?.label,
      };
    },
    [getSelectValue],
  );
