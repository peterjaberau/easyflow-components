...
export const RoleSelector: FC<RoleSelectorProps> = (props) => {
  const { onClickItem, withoutTips, value, showOwner, currentUserRole, isSelf, inline, excludeUserRole } = props;

  const [menuVisible, setMenuVisible] = useState(false);

  const canEdit = !isSelf && isBiggerThanTargetRole(value, currentUserRole, true);
  ...
  const dropUserRole = useMemo(() => {
    const roles = showOwner ? userRoleItems : userRoleItems.filter((i) => i.role !== USER_ROLE.OWNER);

    return roles.filter((i) => !excludeUserRole?.includes(i.role) && isBiggerThanTargetRole(i.role, currentUserRole));
  ...
  return (
    ...
    >
      <div css={roleSelectorRoleContainer}>
        <div css={applyRoleOuterLabelStyle(inline)}>{userRoleItems.find((item) => item.role === value)?.name}</div>
        {canEdit && dropUserRole.length > 1 && (
          <div css={roleOuterIconStyle}>{menuVisible ? <UpIcon /> : <DownIcon />}</div>
        )}
      </div>
    </Dropdown>
  );
