...
const modeHandle = (mode: string, key: number, labelPosition: TimelineLabelPosition) => {
  const classArr = ['alternate-same', 'alternate-relative'];
  // if labelPosition is relative :  1 -> 0, 0 -> 1
  const classIdx = Math.abs((key % 2) - (labelPosition === 'relative' ? 1 : 0));
  ...
  return (
    <TimelineContext.Consumer>
      {(value) => {
        ...
        return (
          <div css={[applyItemStyle(direction, _mode, dot), applyBoxStyle(props)]} ref={ref} {...deleteCssProps(rest)}>
            <div css={applyDotItemStyle(direction, _mode)}>
              {isChildrenLast ? null : <div css={applyLineStyle(direction, lineType, lineColor)} />}
              <div css={applyDotWrapperStyle(direction)}>
                {dot ? dot : <div css={dotCommonStyle(dotColor, dotType)} />}
              </div>
            </div>
            <div css={applyItemContentStyle(direction, _mode, autoFixDotSize)}>{label ? label : children}</div>
          </div>
        );
      }}
    </TimelineContext.Consumer>
