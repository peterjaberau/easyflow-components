...
import { ReduceIcon, SuccessIcon } from '../icon';
import { CheckboxOption, CheckboxProps } from './interface';
import { applyCheckboxSize, applyCheckState, applyMergeCss, childrenContainerCss } from './style';
...
export const Checkbox = forwardRef<HTMLLabelElement, CheckboxProps>((props, ref) => {
  ...
  const finalChecked = checked ?? context.value?.some((item) => item === value);
  const finalDefaultChecked = defaultChecked ?? context.defaultValue?.some((item) => item === value);
  ...
  return (
    <label css={[applyMergeCss(props), applyBoxStyle(props)]} ref={ref} {...deleteCssProps(otherProps)}>
      <input
        type="checkbox"
        css={applyCheckboxSize(currentChecked || indeterminate, colorScheme)}
        value={typeof value === 'object' ? (value as CheckboxOption).value : value}
