...
import { usePrevious } from '../../system';
import debounce from 'lodash.debounce';
import { applyCellStyle, applyCellInnerStyle, listStyle, ulContainerStyle } from './style';
...
export const BaseColumn: FC<TimeColumnProps> = (props) => {
  const { list, value, onHandleSelect, unit, popupVisible, scrollSticky, colorScheme } = props;

  const valueMapLiElement = useRef(new Map<number | string, HTMLLIElement | null>());
  ...
  const prevScrollTop = useRef<number>(scrollContainerRef.current?.scrollTop || 0);
  ...
  useEffect(() => {
    ...
    listItemHeight.current =
      ((ulContainerRef.current?.clientHeight ?? 0) - (scrollContainerRef.current?.clientHeight ?? 0)) /
      (list.length - 1);
  }, [list.length]);
  ...
  const onScrollList = useCallback(
    debounce(() => {
      const mathFunc =
        (scrollContainerRef.current?.scrollTop ?? 0) - prevScrollTop.current > 0 ? Math.ceil : Math.floor;
      const index = mathFunc((scrollContainerRef.current?.scrollTop ?? 0) / listItemHeight.current);
      ...
      <ul ref={ulContainerRef} css={ulContainerStyle}>
        {list.map((item) => {
          return (
            ...
            >
              <div className="cell-inner" css={applyCellInnerStyle(colorScheme, item.selected)}>
                {item.label}
              </div>
            </li>
          );
