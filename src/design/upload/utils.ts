import { isArray, isFunction } from '../system';
...
export const getIconType = (file: UploadItem) => {
  ...
  if (!fileExtension) {
    if (file.originFile && file.originFile.type) {
      fileExtension = (file.originFile.type.split('/').pop() || '').split('.').pop() || '';
    }
  }

  type = fileExtension;
  if (
    ['png', 'jpg', 'jpeg', 'bmp', 'gif', 'svg', 'svg+xml', 'webp', 'jfif', 'dpg', 'ico', 'heic', 'heif'].indexOf(
      fileExtension,
    ) > -1
  ) {
    type = 'image';
    ...
    type = 'audio';
  } else if (['doc', 'docx', 'msword', 'document'].indexOf(fileExtension) > -1) {
    type = 'doc';
  } else if (['xls', 'xlsx', 'sheet', 'ms-excel'].indexOf(fileExtension) > -1) {
    type = 'excel';
  } else if (['ppt', 'pptx', 'ms-powerpoint', 'presentation'].indexOf(fileExtension) > -1) {
    type = 'ppt';
  }
...
export const getFileURL = (file: UploadItem) => {
  ...
  const url =
    file.url ||
    (file?.response && file?.response.hasOwnProperty('url') && (file.response as { url: string }).url) ||
    undefined;
...
export const handleKeyDown = (event: KeyboardEvent<HTMLSpanElement>, callback?: () => void) => {
