...
import { formatValue, verifyRightValue, verifyLeftValue, processNumber } from './utils';
...
export const useOffset = (
  ...
  const getOffsetValueFromState = useCallback(
    (rightVal: number, leftVal?: number) => {
      const rightVerify = verifyRightValue(max, min, step, rightVal, leftVal);
      const leftVerify = verifyLeftValue(max, min, step, rightVal, leftVal);
  ...
  const initOffsetFromState = useCallback(
    (partLength: number, width: number, rightVal: number, leftVal?: number) => {
      const [leftValue, rightValue, barLength] = getOffsetValueFromState(rightVal, leftVal);
      ...
      if (safeRightValue !== rightVal || safeLeftValue !== leftVal) {
        onChange && onChange(safeLeftValue ? [safeLeftValue, safeRightValue] : safeRightValue);
      }
      setLeftOffset(leftValue);
      setBarLength(barLength);
      setRightOffset(rightValue);
      setPartLength(partLength);
      setCurrentValue(safeLeftValue !== undefined ? `${safeLeftValue},${safeRightValue}` : safeRightValue);
      setWidth(width);
    },
    [getOffsetValueFromState, max, min, onChange, step],
  );

  const onDragging = (x: number, startValue: number | number[], location: BarLocation) => {
  ...
  const onRangeDragging = (x: number, startValue: number[], location: string) => {
    let leftBound,
      rightBound,
      val: number,
      tempVal = Math.round(x / partLength) * partLength;
    switch (location) {
      case BarLocation.LEFT: {
        ...
        rightBound = Math.floor((startValue[1] - startValue[0] - step) / step) * partLength;
        if (tempVal >= rightBound) {
          val = rightBound;
        } else if (tempVal <= leftBound) {
          val = leftBound;
        ...
        setBarLength(Math.round((startValue[1] - startValue[0]) / step) * partLength - val);
        break;
      }
      default:
      case BarLocation.RIGHT: {
        leftBound = Math.floor((startValue[0] - startValue[1] + step) / step) * partLength;
  ...
  const onSingleEnd = (
    ...
    const [_, rightVal] = getOffsetValueFromState(currentVal);
  ...
  const onRangeEnd = (
    ...
    switch (location) {
      case BarLocation.RIGHT:
      default: {
        const val = startValue[1] + Math.round(x / partLength) * step;
      ...
      case BarLocation.LEFT: {
        const val = startValue[0] + Math.round(x / partLength) * step;
        if (rightValue - 1 <= val) {
    ...
    const [leftVal, rightVal] = getOffsetValueFromState(currentVal[1], currentVal[0]);
  ...
  const onDragBarEnd = (x: number, startValue: number[], onAfterChange?: (v: number[]) => void) => {
    const currentRangeVal = startValue.map((val, index) => {
      const offset = val + Math.round(x / partLength) * step;
      if (index === 0) {
        const bound = max - (startValue[1] - startValue[0]);
        if (offset <= min) return min;
        else if (offset >= bound) return bound;
        return offset;
      } else {
        const bound = min + (startValue[1] - startValue[0]);
  ...
  const onClickTick = (v: number) => {
    if (Array.isArray(cacheValue)) {
      const mid = Math.floor((cacheValue[1] - cacheValue[0]) / 2) + cacheValue[0];
      if (v > mid) {
        const [_, rightValue, barLength] = getOffsetValueFromState(v, cacheValue[0]);
        setCurrentValue(`${cacheValue[0]},${v}`);
        setRightOffset(rightValue);
        setBarLength(barLength);
      } else {
        const [leftValue, _, barLength] = getOffsetValueFromState(cacheValue[1], v);
