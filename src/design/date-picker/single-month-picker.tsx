...
import { GetHeaderOperationsFun, DatePickerModeType, SingleMonthPickerProps } from './interface';
...
import { dayjsPro, getDayjsValue, getNow, isDayjsChange, toLocal, toTimezone, usePrevious } from '../system';
...
export const SingleMonthPicker = forwardRef<HTMLDivElement, SingleMonthPickerProps>((props, ref) => {
  const {
    allowClear = true,
    placeholder,
    disabled,
  ...
  const realFormat = getFormat('month', props.format) as string;
  const format = realFormat;
  ...
  function onHandleSelect(_: string | undefined, date?: Dayjs) {
    ...
    onSelect && onSelect(localTime ? localTime.format(format) : undefined, localTime as Dayjs);
    setValue(date);
    onHandleChange(date);
    setOpen(false);
  }
  ...
  function onHandleChange(newValue: Dayjs | undefined) {
    if (isDayjsChange(newValue, mergedValue)) {
      onChange && onChange((newValue as Dayjs).format(format), newValue as Dayjs);
    }
  }
  ...
  function isValid(time: string | Dayjs): boolean {
    return (
      ...
      (typeof disabledDate === 'function' ? !disabledDate(dayjsPro(time, format)) : true)
    );
  }
