...
import { throttleByRaf, isFunction, raf, caf, easingMethod } from '../system';
import { BackTopProps } from './interface';
import { applyFixedStyle, applyOpacity, opacityTransition, pointerStyle } from './style';
...
export const BackTop = forwardRef<HTMLDivElement, BackTopProps>((props, ref) => {
    ...
    function updateScrollTopPerFrame() {
      ...
      if (scrollTarget) {
        scrollTarget.scrollTop =
          scrollTop - scrollTop * easingMethod[easing](durationFromStart > duration ? 1 : durationFromStart / duration);
      }

      durationFromStart < duration ? (id = raf(updateScrollTopPerFrame)) : caf(id);
    }
  ...
  return (
    <div
      css={[applyFixedStyle(), pointerStyle, applyOpacity(visible), opacityTransition, applyBoxStyle(props)]}
      ref={ref}
      onClick={scrollToTop}
      {...deleteCssProps(rest)}
    >
