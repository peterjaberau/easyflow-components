...
import { forwardRef, SyntheticEvent, useImperativeHandle, useMemo, useRef, useState } from 'react';
...
export const InputTag = forwardRef<HTMLDivElement, InputTagProps>((props, ref) => {
  const {
    onAdd,
    spaceInput,
    colorScheme = 'blue',
  ...
  const [finalValue, setFinalValue] = useMergeValue<TagObject[] | string[]>([], {
    defaultValue: labelInValue ? (defaultValue as TagObject[]) : (defaultValue as string[]),
      ...
      <>
        {finalValue.map((v, i) => {
          ...
          return (
            <Tag
              css={tagStyle}
              key={labelInValue ? `${i.toString()}:${(v as TagObject).label}` : `${i.toString()}:${value}`}
              visible={true}
              size={size}
            ...
            >
              {renderItem?.(v) ?? (labelInValue ? (v as TagObject).label : (v as string))}
            </Tag>
          );
        })}
      </>
    );
  }, [finalValue, labelInValue, disabled, value, size, renderItem, readOnly, onRemove, onChange, setFinalValue]);
  ...
  const saveInputValue = (e: SyntheticEvent) => {
    if (finalInputValue !== '') {
      ...
      if (labelInValue) {
        ...
        newList = [...(finalValue as TagObject[]), transformValue ? (transformValue(newTag) as TagObject) : newTag];
        onAdd?.(newTag, newList.length - 1, e);
      } else {
        newList = [
          ...(finalValue as string[]),
          (transformValue ? (transformValue(finalInputValue) as string) : finalInputValue) as string,
        ];
  ...
  return (
    <div css={[applyInputContainerStyle(), applyBoxStyle(otherProps)]} ref={ref} {...deleteCssProps(otherProps)}>
      {addBefore && (
        <span
