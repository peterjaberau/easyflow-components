...
export const Steps = forwardRef<HTMLDivElement, StepsProps>((props, ref) => {
  const { children, direction = 'horizontal', lineless, current, type = 'dot', items, onChange, ...otherProps } = props;
  ...
  return (
    <div css={[applyStepsContainerStyle(direction), applyBoxStyle(props)]} ref={ref} {...deleteCssProps(otherProps)}>
      {items &&
        items.map((item, index) => {
          let nodeStatus: StepsStatus = 'wait';
          if (item.status === undefined) {
            nodeStatus = finalCurrent > index ? 'finish' : finalCurrent === index ? 'process' : 'wait';
          } else {
            nodeStatus = item.status;
          }
          let lineStatus: StepsStatus = finalCurrent > index ? 'finish' : 'wait';
          if (index + 1 < items.length) {
            lineStatus = items[index + 1].status === 'error' ? 'error' : lineStatus;
          }
          switch (type) {
            case 'navigation':
              return (
