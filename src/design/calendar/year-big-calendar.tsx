...
export const YearBigCalendar = forwardRef<HTMLDivElement, BigCalendarProps>((props, ref) => {
  ...
  for (let m = 0; m < 12; m++) {
    ...
    const firstDay = currentDate.set('month', m).startOf('month').startOf('week');
    for (let i = 0; i < 42; i++) {
      ...
      monthDay.push(
        <div
          ...
          css={applyMonthBlockDayStyle(
            ...
            currentDay.year() === currentDate.year() && currentDay.month() === m,
    ...
    const title =
      configProviderProps.locale === zhCN || configProviderProps.locale === koKR
        ? `${m + 1}${locale['month']}`
        : getMonthString(m, locale);

    const monthDom = (
      <div className="container" key={`${m} ${currentDate.year()} ${currentDate.month()}`} css={applyMonthBlockStyle()}>
