...
export const CreateFromTemplateModal: FC<CreateFromTemplateProps> = (props) => {
  ...
  const getTemplateList = useCallback(
    async (params: FetchTemplateListParams, needSetMenu: boolean, signal?: AbortSignal) => {
      try {
        setLoading(true);
  ...
  return createPortal(
    <Modal w="896px" visible footer={false} closable withoutPadding onCancel={closeModal}>
      <TemplateHeader
      ...
      <div css={templateContainerStyle}>
        <TemplateMenu
          ...
          onMenuClick={(tag?: string) => {
            ...
            getTemplateList({ hashtags: tag, search: cacheSearch.current }, false);
          }}
        />
        <TemplateContent {...props} loading={loading} templateList={templateList} prevContentNode={prevContentNode} />
      </div>
    </Modal>,
    document.body,
  );
