...
import { AuthShown, SHOW_RULES } from '../../../../AuthShown';
import { ERROR_FLAG, isILLAAPiError } from '../../../../ILLANet';
import { ILLA_MIXPANEL_EVENT_TYPE, MixpanelTrackContext } from '../../../../MixpanelUtils';
import { USER_ROLE } from '../../../../publicTypes';
import { getCurrentTeamInfo, teamActions } from '../../../../UserData';
...
import { fetchRemoveTeamMember, fetchUpdateTeamPermissionConfig } from '../../../service';
import { IPcMoreActionProps } from './interface';
import { allowEditorOrViewerInviteWrapperStyle, moreActionTextStyle } from './style';
...
export const MoreAction: FC<IPcMoreActionProps> = (props) => {
  ...
  const { myRole: currentUserRole, teamMemberID: currentTeamMemberID, id: currentTeamID } = currentTeamInfo;
  const { blockRegister, allowEditorManageTeamMember, allowViewerManageTeamMember } = currentTeamInfo.permission;
  ...
  }, [afterLeaveTeam, currentTeamID, currentTeamMemberID, message, modal, t, track]);
        ...
        onVisibleChange={(show: boolean) => {
          if (show) {
            ...
            track?.(
              ...
              {
                element: 'allow_manage',
                parameter2: allowEditorManageTeamMember && allowViewerManageTeamMember ? 'on' : 'off',
              },
              'both',
            );
          }
          ...
          <DropList>
            ...
            >
              <DropListItem key="1" value="1">
                <div css={allowEditorOrViewerInviteWrapperStyle} onClick={stopPropagation}>
                  <span css={moreActionTextStyle}>{t('user_management.settings.allow_editors_invite')}</span>
                  <Switch
                    colorScheme="techPurple"
                    onClick={stopPropagation}
                    onChange={handleChangeInviteByEditor}
                    disabled={allowInviteLoading}
                    checked={allowEditorManageTeamMember && allowViewerManageTeamMember}
                  />
                </div>
              </DropListItem>
            </AuthShown>
            {!isCloudVersion && (
              ...
              >
                <DropListItem key="2" value="2">
                  <div css={allowEditorOrViewerInviteWrapperStyle} onClick={stopPropagation}>
                    <span css={moreActionTextStyle}>{t('user_management.settings.allow_register')}</span>
                    <Switch
                      colorScheme="techPurple"
                      onClick={stopPropagation}
            ...
            {isCloudVersion && currentUserRole !== USER_ROLE.OWNER ? (
              <DropListItem key="leaveTeam" value="leaveTeam" onClick={handleClickDeleteOrLeaveTeam}>
