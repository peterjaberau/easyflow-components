...
import { ShareAppPC } from '../../../../../InviteModal';
import { ILLA_MIXPANEL_EVENT_TYPE, MixpanelTrackContext, MixpanelTrackProvider } from '../../../../../MixpanelUtils';
import { MemberInfo, USER_STATUS } from '../../../../../publicTypes';
...
import { getILLABuilderURL, getMarketLinkTemplate, isCloudVersion } from '../../../../../utils';
...
export const AppCardActionItem: FC<AppCardActionItemProps> = (props) => {
  ...
  const canEditApp = canManage(teamInfo.myRole, ATTRIBUTE_GROUP.APP, getPlanUtils(teamInfo), ACTION_MANAGE.EDIT_APP);
  ...
  const handleOpenInviteModal = useCallback(() => {
    ...
    if (!openShareAppModal(teamInfo, teamInfo.myRole, appConfig.public, appConfig.publishedToMarketplace)) {
      upgradeModal({
        modalType: 'upgrade',
  ...
  }, [appConfig.public, appConfig.publishedToMarketplace, appID, teamInfo, track, upgradeModal]);
  ...
  const onVisibleChangeForEdit = useCallback(
    (visible: boolean) => {
      if (visible) {
        track?.(ILLA_MIXPANEL_EVENT_TYPE.CLICK, { element: 'app_more', parameter5: appID }, 'both');
        track?.(ILLA_MIXPANEL_EVENT_TYPE.SHOW, { element: 'app_duplicate', parameter5: appID }, 'both');
        track?.(ILLA_MIXPANEL_EVENT_TYPE.SHOW, { element: 'app_delete', parameter5: appID }, 'both');
        canShowShareButton &&
          track?.(ILLA_MIXPANEL_EVENT_TYPE.SHOW, { element: 'invite_entry', parameter5: appID }, 'both');
      }
    },
  ...
  useEffect(() => {
    if (canEditApp || (appDeployed && showInvite)) {
      track?.(ILLA_MIXPANEL_EVENT_TYPE.SHOW, { element: 'app_more', parameter5: appID }, 'both');
    }
  ...
  useEffect(() => {
    appSettingVisible &&
      track?.(ILLA_MIXPANEL_EVENT_TYPE.SHOW, { element: 'app_setting_modal', parameter5: appID }, 'both');
  ...
  return (
    ...
    >
      {canEditApp ? (
        ...
        >
          <Button variant="text" colorScheme="grayBlue" leftIcon={<MoreIcon size="14px" />} />
        </Dropdown>
      ) : (
        canShowShareButton && (
          <Dropdown
      ...
      >
        {shareVisible && isCloudVersion && (
          <ShareAppPC
            ...
            editRedirectURL={`${getILLABuilderURL(window.customDomain)}/${teamInfo.identifier}/app/${appID}`}
            useRedirectURL={`${getILLABuilderURL(window.customDomain)}/${teamInfo.identifier}/deploy/app/${appID}`}
