...
import { getAuthToken, getILLABuilderURL, useIsMobile } from '../../../utils';
...
export const BottomList: FC<BottomListProps> = (props) => {
  ...
  const showTutorial =
    !isMobile &&
    teamIdentifier &&
    [USER_ROLE.ADMIN, USER_ROLE.EDITOR, USER_ROLE.OWNER].includes(currentTeamInfo?.myRole ?? USER_ROLE.VIEWER);
  ...
  const bottomList: MenuItemShape[] = [
    ...
    },
    {
      ...
      labelKey: 'tutorial',
      href: `${getILLABuilderURL(window.customDomain)}/${teamIdentifier}/guide?token=${getAuthToken()}`,
      icon: <TutorialIcon />,
      hidden: !showTutorial,
      onClickCallback: onClickMenuItemCallback,
    },
