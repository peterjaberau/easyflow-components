...
import { dayjsPro, isString } from '../system';
import { FilterOperator } from './interface';

export const transformTableIntoCsvData = (table: Table<any>, multiRowSelection?: boolean) => {
  ...
  table.getRowModel().rows.map((row) => {
    ...
    row.getVisibleCells().map((cell, index) => {
      ...
      const getRenderedValueAsString = cell.column.columnDef.meta?.getRenderedValueAsString;
...
export const transformRawDataIntoCsvData = (table: Table<any>, multiRowSelection?: boolean) => {
  const csvData: Array<Array<any>> = [];
  table.getHeaderGroups().map((headerGroup) => {
    const headerData: unknown[] = [];
    headerGroup.headers.map((header, index) => {
...
export const downloadDataAsCSV = (props: { csvData: Array<Array<any>>; delimiter: string; fileName: string }) => {
...
export const equalTo: FilterFn<any> = (row, columnId: string, filterValue: unknown) => {
...
export const empty: FilterFn<any> = (row, columnId: string, filterValue: string) => {
  const rowValue = row.getValue<string>(columnId);
  return rowValue === '' || rowValue === undefined || rowValue === null;
};

export const notEmpty: FilterFn<any> = (row, columnId: string, filterValue: string) => {
  const rowValue = row.getValue<string>(columnId);
  return rowValue !== '' && rowValue !== undefined && rowValue !== null;
};
...
export const lessThan: FilterFn<any> = (row, columnId: string, filterValue: unknown) => {
  const rowValue = row.getValue<number>(columnId);
  return less(rowValue, filterValue);
};

export const notLessThan: FilterFn<any> = (row, columnId: string, filterValue: unknown) => {
  const rowValue = row.getValue<number>(columnId);
  return !less(rowValue, filterValue);
};
...
export const moreThan: FilterFn<any> = (row, columnId: string, filterValue: unknown) => {
  const rowValue = row.getValue<number>(columnId);
  return more(rowValue, filterValue);
};

export const notMoreThan: FilterFn<any> = (row, columnId: string, filterValue: unknown) => {
  const rowValue = row.getValue<number>(columnId);
  return !more(rowValue, filterValue);
};

export const contains: FilterFn<any> = (row, columnId: string, filterValue: unknown) => {
  const rowValue = row.getValue<string>(columnId);
...
export const doesNotContain: FilterFn<any> = (row, columnId: string, filterValue: unknown) => {
  const rowValue = row.getValue<string>(columnId);
  if (isString(rowValue) && isString(filterValue)) {
    return !rowValue.includes(filterValue);
  }
...
export const before: FilterFn<any> = (row, columnId: string, filterValue: unknown) => {
  const rowValue = row.getValue<string>(columnId);
...
export const after: FilterFn<any> = (row, columnId: string, filterValue: unknown) => {
  const rowValue = row.getValue<string>(columnId);
  if (isString(filterValue)) {
