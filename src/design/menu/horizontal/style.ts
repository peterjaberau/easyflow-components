...
export function applyActionContainerStyle(action: 'left' | 'right'): SerializedStyles {
  const positionStyle =
    action === 'left'
      ? css`
          left: 0;
...
export function applyHorizontalSubMenuIcon(onlyShowIcon?: boolean): SerializedStyles {
  return css`
    font-size: 16px;
    display: inline-flex;
    align-items: center;
