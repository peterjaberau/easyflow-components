...
export function createIcon(options: CreateIconOptions) {
  const { viewBox = '0 0 24 24', d: pathDefinition, path, title, fill = 'none', defaultProps = {} } = options;
  ...
  return forwardRef<SVGSVGElement, IconProps>((props, ref) => {
    ...
    return (
      <span css={[iconContainerStyle, containerStyle]}>
        <Icon ref={ref} viewBox={viewBox} fill={fill} {...defaultProps} {...rest}>
          <title>{title}</title>
          {path ? Children.toArray(path) : <path fill="currentColor" d={pathDefinition} />}
        </Icon>
      </span>
    );
  });
