import { ClickhouseSSL, DbSSL, MicrosoftSqlSSL, ResourceType } from '../../../publicTypes';
...
const getSSLConfig = (data: { [p: string]: any }, type?: ResourceType): AllSSLConfigType => {
  switch (type) {
    case 'mssql':
      return {
        ssl: true,
...
export function generateSSLConfig(
  ...
  return open ? getSSLConfig(data, type) : SSLConfigDefaultValue[type || ''] || DbSSLInitial;
}
