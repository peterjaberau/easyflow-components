...
import { Select, Skeleton, Space, Tag } from '../../../../../design/react';
...
import { fetchAgentDetailInfoByAgentID, fetchAppDetailInfoByAppID, fetchRecommendHashtag } from '../service';
import { recommendLabelStyle, tagContainer, tagInputContainerStyle, titleStyle } from './style';
...
export const TagControllerPC: FC<TagControllerProps> = (props) => {
  ...
  const [searchRecommendTagsLoading, setSearchRecommendTagsLoading] = useState(false);
        ...
        <Space wrap>
          {recommendTags.map((tag) => (
            <Tag
              ...
              key={tag}
              colorScheme={currentHashtags.includes(tag) ? 'techPurple' : 'grayBlue'}
              onClick={() => {
