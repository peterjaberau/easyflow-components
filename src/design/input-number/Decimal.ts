...
export class NumberDecimal {
  ...
  add(value: string | number): NumberDecimal {
    ...
    const maxPrecision = Math.max(getNumberPrecision(this.number), getNumberPrecision(offset.number));
...
export function toFixed(numStr: string, precision?: number, cutOnly = false): string {
  ...
  if (precision && precision >= 0) {
    ...
    if (advancedNum >= 5 && !cutOnly) {
      const advancedDecimal = getDecimal(numStr).add(`${negativeStr}0.${'0'.repeat(precision)}${10 - advancedNum}`);
      return toFixed(advancedDecimal.toString(), precision, cutOnly);
    }

    return precision === 0
      ? numberWithoutDecimal
      : `${numberWithoutDecimal}${separator}${decimalStr.padEnd(precision, '0').slice(0, precision)}`;
  }
