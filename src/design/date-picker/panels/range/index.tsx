...
import { getDayjsValue, getNow, isObject } from '../../../system';
...
export const RangePickerPanel: FC<RangePickerPanelProps> = (props) => {
  ...
  const value = getDayjsValue(propsValue as Dayjs[], format as string) as Dayjs[];
      ...
      return (
        <>
          <WeekPickerPanel
            ...
            setPanelMode={(mode) => setPanelModes?.([mode, panelModes?.[1] ?? mode])}
          />
          <WeekPickerPanel
            ...
            setPanelMode={(mode) => setPanelModes?.([panelModes?.[0] ?? mode, mode])}
          />
        </>
      );
    }
