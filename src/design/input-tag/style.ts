...
import { getColor, getColorShadow, getSpecialThemeColor, zIndex } from '../theme';

export function applyInputTagInputStyle(size: InputTagSize, w: number): SerializedStyles {
  return css`
    width: ${w}px;
    font-size: inherit;
    background: none;
...
export function applyInputTagPlaceHolderStyle(size: InputTagSize): SerializedStyles {
  return css`
    color: ${getColor('grayBlue', '04')};
    width: unset;
    white-space: nowrap;
    ...
    line-height: ${size === 'large' ? '32px' : size === 'small' ? '22px' : '22px'};
  `;
}
...
export function applyInputTagContainerStyle(
  ...
  if (!disabled) {
    otherStyle = css`
      &:hover {
        border-color: ${error ? getColor('red', '02') : getColor(colorScheme, '07')};
  ...
  return css`
    ...
    border-radius: ${!hasBefore ? borderList[0] : '0'} ${!hasAfter ? borderList[1] : '0'}
      ${!hasAfter ? borderList[2] : '0'} ${!hasBefore ? borderList[3] : '0'};
    border: 1px solid ${error ? getColor('red', '03') : getColor('grayBlue', '08')};
    flex-grow: 1;
    flex-shrink: 1;
    background-color: ${disabled ? getColor('grayBlue', '09') : 'unset'};
    ${getPaddingStyle(size)};
...
export function applyPrefixSuffixStyle(disabled: boolean, marginStyle: SerializedStyles): SerializedStyles {
  return css`
    align-self: center;
    height: 100%;
    display: flex;
