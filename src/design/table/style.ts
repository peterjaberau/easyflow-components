...
import { getColor, getSpecialThemeColor, globalColor, illaPrefix, zIndex } from '../theme';
import { TableAlign, TableLayout, TableSize } from './interface';

export function applySizeStyle(size: TableSize, customCellPadding?: string): SerializedStyles {
  let paddingStyle;
  switch (size) {
    case 'small':
      paddingStyle = css`
...
export const applyOverflowContentStyle = (lastRow?: boolean, lastCol?: boolean) => {
  return css`
...
export function applyActionButtonStyle(showPagination?: boolean): SerializedStyles {
  return css`
    right: 16px;
...
export const applyResizerTableHeaderStyle = (enableColumnResizing?: boolean) => {
...
export const applyTableCellBackgroundStyle = (color?: string): SerializedStyles => {
  if (color) {
    return css`
