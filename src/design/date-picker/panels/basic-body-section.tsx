      ...
      {rows?.map((row, rowIndex) => (
        <div key={rowIndex} css={bodyRowSectionStyle}>
          {row.map((col, colIndex) => {
            if (col.time) {
              const disabled = isDisabledDate(col.time, mode, disabledDate, originMode);
              ...
              const onClickHandler = () => !disabled && onSelectDate?.(col.time!.format(format) as string, col.time!);
              return (
                <div key={colIndex} css={applyCellStyle(cellStatus)} onClick={onClickHandler}>
                  {dateRender ? (
                    dateRender(col.time)
                  ) : (
                    <div css={applyDateCellStyle(cellStatus)} className="date-cell">
                      <div
