...
import { CascaderItem, CascaderOptionObject, SelectorOption, SelectorProps } from './interface';
import { Checkbox } from '../checkbox';
import { applyListSelectorItemTextStyle, applyListSelectorSingleItemStyle, listSelectorContainerStyle } from './style';
import { Empty } from '../empty';
import { traversalOptions } from './util';

export const ListSelector = forwardRef<HTMLDivElement, SelectorProps>((props, ref) => {
  const { multiple, value = [], options = [], onChange, inputValue = '', filterOption, colorScheme, onClick } = props;
    ...
    return filterOptions.map((v) => {
      ...
      return {
        ...
        labelCollection: v
          ...
          .join(' / '),
        checked: valueCollection.some((value) => value === currentValueString),
      } as CascaderItem;
    });
  }, [filterOptions, value]);
          ...
          >
            {multiple && (
              <Checkbox
                ...
                onChange={(checked) => {
                  if (checked) {
                    onChange?.([...value, item.selectorOptions.map((item) => item.value)]);
                  } else {
                    onChange?.(
                      value.filter((i) => {
            ...
            <span css={applyListSelectorItemTextStyle(item.checked ?? false)}>{item.labelCollection}</span>
          </div>
        );
      })}
