...
export const SingleSelect = forwardRef<HTMLInputElement, SelectProps>((props, ref) => {
  const {
    size = 'medium',
    allowClear,
    onSelect,
  ...
  const getValueFromProps = useCallback(
    (dealValue?: SelectOptionObject | string | SelectOptionObject[] | string[] | number | number[] | ReactNode) => {
        ...
        if (options === undefined) {
          ...
          if (labelInValue) {
            dV = (options as SelectOptionObject[]).find(
              (option) => option.value === (dealValue as SelectOptionObject).value,
            )?.label;
          } else {
            if (options.length > 0) {
              if (typeof options[0] === 'string' || typeof options[0] === 'number') {
                ...
                dV = (options as SelectOptionObject[]).find((option) => option.value === dealValue)?.label;
              }
            }
          }
        }
  ...
  const [finalValue, setFinalValue] = useMergeValue<number | string | ReactNode | undefined>('', {
    defaultValue: getValueFromProps(defaultValue),
    value: getValueFromProps(value),
  });

  const [finalInputValue, setFinalInputValue] = useState<number | string | ReactNode | undefined>(
    value === undefined ? getValueFromProps(defaultValue) : getValueFromProps(value),
  );
  ...
  const [finalSelectValue, setFinalSelectValue] = useMergeValue<SelectValue | undefined>(undefined, {
  ...
  const finalOptions: SelectOptionObject[] = useMemo(() => {
    ...
    if (inputAsOption) {
      ...
      if (lastChooseRef.current && lastChooseRef.current !== '') {
        const optionIndex = newOptions.findIndex((option) => option.value === lastChooseRef.current);
        if (optionIndex !== -1) {
          newOptions.splice(optionIndex, 1);
        }
        newOptions = [
    ...
    if (
      ...
      finalInputValue &&
      finalInputValue !== '' &&
      (typeof finalInputValue === 'string' || typeof finalInputValue === 'number')
    ) {
      newOptions = newOptions.filter((option) => {
        ...
        return (
          ...
          option.label.toLowerCase().includes(finalInputValue.toString().toLowerCase())
        );
      });
    } else {
      if (typeof finalInputValue === 'string' || typeof finalInputValue === 'number') {
        newOptions = newOptions.filter((option) => {
    ...
    return newOptions;
  }, [defaultFilterOption, filterOption, finalInputValue, inputAsOption, options, showSearch]);
  ...
  return (
    ...
    >
      <Input
        ...
        prefix={prefix}
        placeholder={placeholder ?? (lastChooseRef.current !== undefined ? String(lastChooseRef.current) : undefined)}
        onChange={(v) => {
