...
export const Bar = forwardRef<HTMLDivElement, SliderBar>((props, ref) => {
  const { isRange, dragBar, value, left, width, disabled, containerWidth, partLength, colorScheme, onDragBarEnd } =
    props;
  ...
  const onDrag = (_: any, info: PanInfo) => {
    if (!isRange || !(isObject(isRange) && isRange.draggableBar) || disabled) return;
    const {
      offset: { x },
    } = info;
  ...
  return (
    <motion.div
      drag={!disabled && isObject(isRange) && isRange.draggableBar ? 'x' : false}
      ref={ref}
      onDragStart={onDragStart}
      onDrag={onDrag}
      onDragEnd={onDragEnd}
      css={applySliderBar(disabled, isObject(isRange) && isRange.draggableBar, width, colorScheme)}
      dragElastic={false}
