...
import { CloseIcon, Modal, TabPane, Tabs } from '../../../../design/react';
...
export const ShareAppPC: FC<ShareAppProps> = (props) => {
  ...
  if (props.canInvite && isBiggerThanTargetRole(USER_ROLE.VIEWER, props.currentUserRole, false)) {
    defTab = 'edit';
  } else if (props.canInvite && USER_ROLE.VIEWER === props.currentUserRole && props.isDeployed) {
    defTab = 'use';
  } else if (
    props.canPublic &&
  ...
  return (
    ...
    >
      <div css={headerContainerStyle}>
        ...
        >
          {props.canInvite && isBiggerThanTargetRole(USER_ROLE.VIEWER, props.currentUserRole, false) && (
            <TabPane title={t('user_management.modal.title.invite_to_edit')} key="edit" />
          )}
          {props.canInvite && props.isDeployed && (
            <TabPane title={t('user_management.modal.title.invite_to_use')} key="use" />
          )}
          {props.canPublic &&
            (isBiggerThanTargetRole(USER_ROLE.VIEWER, props.userRoleForThisApp, false) ||
              props.defaultAppContribute ||
              props.defaultAppPublic) &&
            isCloudVersion && <TabPane title={t('user_management.modal.tab.public')} key="public" />}
        </Tabs>
        <div
          css={closeIconStyle}
          onClick={() => {
