import { generateBaseActionItem } from '../../../publicConfig';
import { ACTION_RUN_TIME, ActionType } from '../../../publicTypes';
import { v4 } from 'uuid';
...
const buildAddRowAction = (
  ...
  const inputQuery = rows
    ...
    .join(',');
  const query = `INSERT INTO ${modelName} (${rows.map(({ name }) => name).join(',')}) values (${inputQuery})`;
...
export const buildActionInfo = (
  ...
  const listDataAction = buildListDataAction(actionType, resourceID, modelName, showRows);
  const deleteRowAction = buildDeleteRowAction(actionType, resourceID, modelName, showRows);
  const updateRowAction = buildUpdateRowAction(actionType, resourceID, modelName, showRows, editInputDisplayNames);

  const addRowAction = buildAddRowAction(actionType, resourceID, modelName, showRows, addInputDisplayNames);
