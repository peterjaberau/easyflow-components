...
function getTrList(data: DescriptionItem[], columnNumber: number): DescriptionItem[][] {
  let currentColumn = columnNumber;
  const trList: DescriptionItem[][] = [];
        ...
        <Fragment key={tr.indexOf(td)}>
          ...
          >
            <div css={applyInlineHorizontalStyle}>
              <span css={applyLabelStyle(bordered ?? false, layout)}>{td.label}</span>
...
function applyVerticalLayout(
  ...
  for (const tr of finalData) {
    ...
    tableContent.push(<tr key={finalData.indexOf(tr) + 'second'}>{tdSecond}</tr>);
  }
  return tableContent;
}
...
export const Description = forwardRef<HTMLDivElement, DescriptionProps>((props, ref) => {
  ...
  const finalData = data != undefined ? getTrList(data, getColumn(column, width)) : [];

  let tableContent: ReactNode[];
  switch (layout) {
    case 'horizontal':
      tableContent = applyHorizontalLayout(finalData, align, bordered ?? false, size, layout, getColumn(column, width));
      break;
    case 'inline-horizontal':
      ...
      break;
    case 'vertical':
      tableContent = applyVerticalLayout(finalData, align, bordered ?? false, size, layout, getColumn(column, width));
      break;
  ...
  return (
    <div ref={ref} css={[applyDescContainerStyle, applyBoxStyle(props)]} {...deleteCssProps(otherProps)}>
