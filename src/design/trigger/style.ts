...
import { getColor, getColorShadow, getSpecialThemeColor, globalColor, illaPrefix } from '../theme';
...
export function applyTipsText(
  ...
  const textColor =
    colorScheme == 'white' ? globalColor(`--${illaPrefix}-grayBlue-02`) : globalColor(`--${illaPrefix}-white-01`);
...
export function applyAnimation(position: TriggerPosition, showArrow: boolean): Variants {
  const isHorizontal =
    position == 'left' ||
    position == 'left-start' ||
    position == 'left-end' ||
  ...
  switch (position) {
    ...
    case 'top-end':
      return getAnimation(`calc(100% - 12px)`, `calc(100%)`, showArrow, isHorizontal);
    case 'bottom':
    ...
    case 'left-end':
      return getAnimation(`calc(100%)`, `calc(100% - 12px)`, showArrow, isHorizontal);
    case 'right':
...
export function applyVerticalContainer(autoAlignPopupWidth?: boolean): SerializedStyles {
  return css`
    display: inline-flex;
...
export function applyHorizontalContainer(autoAlignPopupWidth?: boolean): SerializedStyles {
  return css`
    display: inline-flex;
    flex-direction: row;
    width: ${autoAlignPopupWidth ? '100%' : 'auto'};
