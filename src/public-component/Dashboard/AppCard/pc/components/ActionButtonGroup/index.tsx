...
import { getAuthToken, getILLABuilderURL } from '../../../../../utils';
import { FC, MouseEvent, useContext, useMemo } from 'react';
import { useTranslation } from 'react-i18next';
import { useParams } from 'react-router-dom';
import { Button, Space } from '../../../../../../design/react';
...
export const ActionButtonGroup: FC<ActionButtonGroupProps> = (props) => {
  ...
  const toDeployedApp = useMemo(
    () => (e: MouseEvent) => {
      ...
      window.open(
        `${getILLABuilderURL(window.customDomain)}/${teamIdentifier}/deploy/app/${appID}?token=${getAuthToken()}`,
        '_blank',
      );
    },
  ...
  const toEditApp = useMemo(
    () => (e: MouseEvent) => {
      ...
      window.open(
        `${getILLABuilderURL(window.customDomain)}/${teamIdentifier}/app/${appID}?token=${getAuthToken()}`,
        '_blank',
      );
    },
    [appID, teamIdentifier, track],
  ...
  return (
    <Space direction="horizontal" w="100%" justifyContent="end" size="8px" alignItems="center">
      {appDeployed ? (
        <Button
