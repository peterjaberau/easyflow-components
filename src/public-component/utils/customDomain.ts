...
export async function validateCustomDomain() {
  ...
  if (origin !== '%ILLA_CLOUD_URL%') {
    try {
      const response = await fetch(`https://%ILLA_API_BASE_URL%/supervisor/api/v1/domain/validate`, {
        method: 'POST',
        headers: {
      ...
      if (!data.customDomainAvaliable) {
        const redirectURL = window.location.href.replace(`https://${window.location.host}/cloud`, '%ILLA_CLOUD_URL%');
...
export default { validateCustomDomain };
