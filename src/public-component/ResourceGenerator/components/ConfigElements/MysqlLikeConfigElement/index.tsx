...
import { MysqlLikeResource, Resource } from '../../../../publicTypes';
...
function parseDatabaseConnectionString(connectionString: string): Omit<MysqlLikeResource, 'ssl'> | undefined | any {
  const regex = /^(.*):\/\/(.*):(.*)@(.*?)(?::(\d+))?\/(.*)$/;
...
const MysqlLikeConfigElement: FC<MysqlLikeConfigElementProps> = (props) => {
  ...
  const findResource = getResourceByID(resourceID) as Resource<MysqlLikeResource>;

  const sslDefaultValue = findResource?.content.ssl.ssl ?? (resourceType === 'tidb' || resourceType === 'hydra');
  const serverCertDefaultValue =
    findResource?.content.ssl.serverCert ??
    (resourceType === 'tidb' || resourceType === 'hydra' ? tiDBServertCertDefaultValue : '');
          ...
          <TextLink
            ...
            onClick={() => {
              if (resourceType === 'tidb') {
                window.open('https://docs.pingcap.com/tidbcloud/tidb-cloud-tls-connect-to-dedicated-tier', '_blank');
              } else if (resourceType === 'hydra') {
                window.open('https://docs.hydra.so/cloud-warehouse-operations/tls', '_blank');
              }
            }}
          />,
        ]}
  ...
  const handleDocLinkClick = () => window.open('https://www.illacloud.com/docs/illa-cli', '_blank');
    ...
    <>
      <div css={container}>
        ...
        <ControlledElement
          ...
          placeholders={[t('editor.action.resource.db.placeholder.hostname'), getResourceDefaultPort(resourceType)]}
        />
        {showAlert && (
          <ControlledElement
            defaultValue=""
                ...
                content={
                  isCloudVersion ? (
                    <Trans
                      ...
                      t={t}
                      components={[
                        <TextLink key="editor.action.form.tips.connect_to_local.cloud" onClick={handleDocLinkClick} />,
                      ]}
                    />
                  ) : (
        ...
        <ControlledElement
          ...
          defaultValue={[findResource?.content.databaseUsername, findResource?.content.databasePassword]}
