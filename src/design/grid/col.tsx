...
export const Col = forwardRef<HTMLDivElement, ColProps>((props, ref) => {
  const { span = 24, offset = 0, order, push = 0, pull = 0, xs, sm, md, lg, xl, xxl, ...otherProps } = props;
  ...
  // reactive css
  const xsCss = applyReactiveStyle('0px', oneUnitWidth.xs ?? oneUnitWidth.normal, xs);
  const smCss = applyReactiveStyle('576px', oneUnitWidth.sm ?? oneUnitWidth.normal, sm);
  const mdCss = applyReactiveStyle('768px', oneUnitWidth.md ?? oneUnitWidth.normal, md);
  const lgCss = applyReactiveStyle('992px', oneUnitWidth.lg ?? oneUnitWidth.normal, lg);
  const xlCss = applyReactiveStyle('1200px', oneUnitWidth.xl ?? oneUnitWidth.normal, xl);
  const xxlCss = applyReactiveStyle('1600px', oneUnitWidth.xxl ?? oneUnitWidth.normal, xxl);
  ...
  return (
    <div ref={ref} css={[finalCss, applyBoxStyle(props)]} {...deleteCssProps(otherProps)}>
      {props.children}
    </div>
  );
});
