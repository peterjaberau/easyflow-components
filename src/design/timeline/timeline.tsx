...
export const Timeline = forwardRef<HTMLDivElement, TimelineProps>((props, ref) => {
  ...
  const pendingItem = pending ? (
    <TimelineItem dot={pendingDot || pendingNode} mode={mode} direction={direction} key={items.length} />
  ) : null;
  pending && items.push(pendingItem as ReactElement);

  return (
    <div ref={ref} css={css(applyWrapStyle(direction, mode), applyBoxStyle(props))} {...deleteCssProps(rest)}>
