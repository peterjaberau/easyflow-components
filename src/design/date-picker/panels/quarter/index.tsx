...
import { dayjsPro } from '../../../system';

const isSameTime = (current: Dayjs, target: Dayjs) => current.isSame(target, 'month');

const generateRows = (showYear: number) => {
  return [
    new Array(4).fill(0).map((_, index) => ({
      name: `Q${index + 1}`,
      time: dayjsPro(`${showYear}-${padStart(index * 3 + 1, 2, '0')}`, 'YYYY-MM'),
    })),
  ];
};
export const QuarterPickerPanel: FC<QuarterPickerPanelProps> = (props) => {
  ...
  const rows = generateRows(pageShowDate ? pageShowDate.year() : dayjsPro().year());
