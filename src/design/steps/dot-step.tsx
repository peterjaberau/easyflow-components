...
export const DotStep = forwardRef<HTMLDivElement, StepProps>((props, ref) => {
  ...
  if (direction === 'horizontal') {
    return (
      <div ref={ref} css={applyHorizontalStepContainerStyle(canClick, disabled, last)} {...deleteCssProps(otherProps)}>
        <div css={horizontalStatusContainerStyle}>
          {statusElement}
          <div css={horizontalTitleStyle}>{title}</div>
          {!last && <div css={applyHorizontalLineStyle(lineStatus, lineless)} />}
        </div>
        {description && <div css={applyHorizontalDescriptionStyle(type)}>{description}</div>}
      </div>
    );
  } else {
    return (
      <div ref={ref} css={applyVerticalStepContainerStyle(canClick, disabled, last)} {...deleteCssProps(otherProps)}>
        <div css={verticalStatusContainerStyle}>
          {statusElement}
          <div css={verticalTitleStyle}>{title}</div>
        </div>
        <div css={verticalBottomContainer}>
          {!last && <div css={applyVerticalLineStyle(lineStatus, type, lineless)} />}
          {description && <div css={applyVerticalDescriptionStyle(type)}>{description}</div>}
        </div>
      </div>
    );
  }
