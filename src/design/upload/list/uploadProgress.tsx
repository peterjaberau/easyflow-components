...
import { UploadIcon, SuccessIcon } from '../../icon';
import { isFunction } from '../../system';
import { ConfigProviderContext, ConfigProviderProps, def } from '../../config-provider';
import { successIconStyle, uploadProgressFailStyle, uploadProgressStatus } from '../style';
...
const UploadProgress: FC<UploadProgressProps> = (props) => {
  ...
  const progressStatus = status === STATUS.FAIL ? 'error' : status === STATUS.SUCCESS ? 'success' : 'normal';
  ...
  const dom = (
    <>
      {status === STATUS.FAIL && props.reuploadIcon !== null && (
        ...
        >
          {props.reuploadIcon || (props.listType === 'picture-card' ? <UploadIcon /> : locale.retry)}
        </span>
      )}

      {status === STATUS.SUCCESS && props.successIcon !== null && (
        <span css={successIconStyle}>{props.successIcon || <SuccessIcon />}</span>
      )}
