...
import { getIconFromResourceType } from '../../Icon';
import { ILLA_MIXPANEL_EVENT_TYPE, MixpanelTrackContext } from '../../MixpanelUtils';
import { Resource } from '../../publicTypes';
import { ResourceGenerator, ResourceGeneratorProvider } from '../../ResourceGenerator';
import { getCurrentTeamInfo } from '../../UserData';
...
import { CreateWithResourceProps, ISchemaData, RESOURCE_TYPE } from './interface';
...
export const CreateFromResourceModal: FC<CreateWithResourceProps> = ({
  ...
  const selectResource = useMemo(() => {
    return resourceList.find((resource) => resource.resourceID === selectResourceID);
  ...
  const showModelList = useMemo(() => {
    ...
    if (isObject(model) && model !== null) {
      return Object.keys(model).map((key) => ({
        ...
        inputType: getInputType(selectResource.resourceType as RESOURCE_TYPE, model[key].data_type || ''),
      }));
    }
  ...
  const getDetails = useCallback(
    async (resourceID?: SelectValue) => {
      ...
      try {
        ...
        const res = await fetchResourceMeta(resourceID as string, teamID, controller.signal);
        const data = res?.data?.Schema as Record<string, ISchemaData>;
        const schemeList: [string, ISchemaData][] = Object.keys(data).map((key) => [key, data[key]]);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const { appInfo, editInputDisplayNames, addInputDisplayNames } = buildAppWithResourceSchema(
  ...
  return createPortal(
    <>
      <div css={modalContainerStyle}>
        <Modal w="auto" visible footer={false} closable withoutPadding onCancel={closeModal}>
          ...
          <div css={containerStyle}>
            ...
            <div css={contentContainerStyle}>
              {getResourceLoading || loading ? (
                ...
                <DatasetTable closeModal={closeModal} rows={showModelList || []} dataGridRef={dataGridRef} />
              ) : (
                <div css={emptyStyle}>
                  <Empty />
                </div>
            ...
            <div css={footerStyle}>
              <Button w="200px" loading={createLoading} colorScheme="techPurple" onClick={handleCreate}>
      ...
      <TriggerProvider zIndex={BIGGER_THAN_MODAL_RESOURCE}>
        <ResourceGeneratorProvider allResource={resourceList} createOrUpdateResourceCallback={createResourceCallback}>
          <ResourceGenerator
            ...
            filterResourceType={(resourceType) => Object.values(RESOURCE_TYPE).includes(resourceType as RESOURCE_TYPE)}
          />
