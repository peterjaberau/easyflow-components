...
export function applyLabelStyle(bordered: boolean, layout: DescriptionLayout): SerializedStyles {
  switch (layout) {
    case 'horizontal':
      if (bordered) {
        return css`
...
export function applyBlockStyle(size: DescriptionSize, bordered: boolean, column: number): SerializedStyles {
  let padding;
...
export function applyTableStyle(tableLayout: DescriptionTableLayout, bordered: boolean): SerializedStyles {
  const border = css`
    border: solid 1px ${globalColor(`--${illaPrefix}-grayBlue-08`)};
  `;
