import { getColor, getSpecialThemeColor, globalColor, illaPrefix } from '../theme';
import {
  RadioColorScheme,
  RadioGroupDirection,
  RadioGroupSpacing,
...
// default radio
export function applyRadioSize(colorScheme: RadioColorScheme): SerializedStyles {
  return css`
    position: relative;
    appearance: none;
    border-radius: 50%;
...
export function applyRadioButtonContainer(size: RadioSize, hasChildren?: ReactNode): SerializedStyles {
  return css`
...
export const applyRadioContainerCss = (stateValue: RadioStatus, type?: RadioGroupType): SerializedStyles => {
  return type === 'button' ? applyRadioButton(stateValue) : applyMergeCss(stateValue);
};

export const applyRadioCss = (colorScheme: RadioColorScheme, type?: RadioGroupType): SerializedStyles => {
