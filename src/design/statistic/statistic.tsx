...
export const Statistic = forwardRef<any, StatisticProps>((props, ref) => {
  ...
  return (
    <div css={[statisticStyle, applyBoxStyle(props)]} {...deleteCssProps(restProps)}>
      {title && <div css={statisticTitleStyle(colorScheme)}>{title}</div>}
      <div>
        <Skeleton animation visible={!!loading} text={{ rows: 1, width: '100%' }}>
          <div css={statisticContentStyle(colorScheme)}>
            {prefix && <span css={applyStatisticDecoratorStyle(true, !isObject(prefix))}>{prefix}</span>}
            <span css={statisticValueStyle}>{renderValue.toString()}</span>
            {suffix && <span css={applyStatisticDecoratorStyle(false, !isObject(suffix))}>{suffix}</span>}
          </div>
        </Skeleton>
