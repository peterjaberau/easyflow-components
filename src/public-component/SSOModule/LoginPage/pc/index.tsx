...
export const PCLogin: FC<loginProps> = (props) => {
  ...
  const { onSubmit, errorMsg, loading, lockedEmail, hideOAuth, hideRegister } = props;
  ...
  useEffect(() => {
    if (asyncValid && !asyncValid.isValid) {
      validateReport(ILLA_MIXPANEL_PUBLIC_PAGE_NAME.LOGIN, 'sign_in', false, errors);
    }
  }, [asyncValid, errors]);
      ...
      <form css={gridFormStyle} onSubmit={handleSubmit(onSubmit)}>
        ...
        <section css={gridFormFieldStyle}>
          <section css={gridItemStyle}>
            <label css={formLabelStyle}>{t('page.user.sign_in.fields.email')}</label>
                ...
                rules={{
                  ...
                  validate: (value: string) => {
                    if (isCloudVersion && !EMAIL_FORMAT.test(value)) {
                      return t('page.user.sign_up.error_message.email.invalid_pattern');
                    }
                    return value === 'root'
                      ? true
          ...
          <section css={gridItemStyle}>
            <div css={forgotPwdContainerStyle}>
              <label css={formLabelStyle}>{t('page.user.sign_in.fields.password')}</label>
              {isCloudVersion && (
                <TextLink
                  css={forgotPwdStyle}
                  onClick={() => {
      ...
      {isCloudVersion && !hideOAuth && (
        <div>
          ...
          <div css={oAuthButtonGroupStyle}>
            <OAuthButton icon={<GithubIcon css={oAuthIconStyle} />} type="github" isMobile={false} landing="signin">
