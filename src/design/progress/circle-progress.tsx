...
export const CircleProgress = forwardRef<HTMLDivElement, ProgressProps>((props, ref) => {
  ...
  switch (status) {
    case 'success':
      ...
      break;
    case 'error':
      statusComponent = <WarningIcon color={globalColor(`--${illaPrefix}-red-03`)} />;
      break;
  }

  return (
    <div css={[applyContainer(w), applyBoxStyle(props)]} ref={ref} {...deleteCssProps(otherProps)}>
      ...
      {status == 'normal' && showText && <span css={applyProgressText}>{formatText(percent)}</span>}
