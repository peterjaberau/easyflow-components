...
import { Button, ButtonColorScheme, ButtonVariant, Progress, getColor } from '../../design/react';
...
export const UsageCard: FC<UsageCardProps> = (props) => {
  ...
  const percent = useMemo(() => Math.round((current / total) * 100), [total, current]);

  const { cardStyle, firstLineStyle, iconTypeStyle, progressStyle, lastActionsStyle, buttonStyle } = useMemo(() => {
    if (isMobile)
      return {
  ...
  return (
    <div css={cardStyle} {...rest}>
      ...
      <Progress
        _css={progressStyle}
        color={percent > 75 ? getColor('red', '03') : getColor('techPurple', '03')}
        w={'100%'}
      ...
      <div css={lastActionsStyle}>
        ...
        <Button _css={buttonStyle} colorScheme={buttonColorScheme} variant={buttonVariant} onClick={onClick}>
