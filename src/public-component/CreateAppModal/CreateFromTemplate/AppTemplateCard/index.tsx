...
import { Button, Loading, applyBoxStyle } from '../../../../design/react';
      ...
      <div css={cardImageContainerStyle(size)}>
        ...
        <div css={cardMaskStyle(showMask)} onClick={forkApp}>
          {loading ? (
            ...
            <Button colorScheme="techPurple">{t('new_dashboard.create_new.use_this_template')}</Button>
          )}
        </div>
      </div>
      <div css={cardContentContainerStyle}>
        <span css={appNameStyle(size)}>{appName}</span>
        {showAppDescription && <span css={appDescriptionStyle}>{appDescription}</span>}
      </div>
    </div>
  );
};
