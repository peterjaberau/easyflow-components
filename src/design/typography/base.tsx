...
import { Children, FC, Fragment, MutableRefObject, useContext, useEffect, useRef, useState } from 'react';
import {
...
import { SuccessCircleIcon, CopyIcon } from '../icon';
...
function getEllipsis(configProviderProps: ConfigProviderProps, ellipsis?: boolean | Ellipsis): Ellipsis {
  let originEllipsis: Ellipsis | any;
...
function getCopyable(configProviderProps: ConfigProviderProps, copyable?: boolean | Copyable): Copyable {
  // get copyable
  let originCopyable: Copyable | any;
  ...
  if (originCopyable.copyIcon == undefined) {
    originCopyable.copyIcon = <CopyIcon color={globalColor(`--${illaPrefix}-grayBlue-01`)} />;
  }
  if (originCopyable.copiedIcon == undefined) {
    originCopyable.copiedIcon = <SuccessCircleIcon color={globalColor(`--${illaPrefix}-green-03`)} />;
  }
...
export const Base: FC<BaseProps> = (props) => {
  // get props
  const { colorScheme = 'gray', ellipsis, bold, disabled, mark, underline, deleted, copyable } = props;

  const configContext = useContext<ConfigProviderProps>(ConfigProviderContext);

  const originEllipsis = getEllipsis(configContext, ellipsis);
  const originCopyable = getCopyable(configContext, copyable);
  ...
  const copyablePanel =
    copyable && originCopyable.copyIcon && showCopyTooltip ? (
      <Trigger closeOnClick={false} content={copied ? originCopyable.copiedToolTip : originCopyable.copyToolTip}>
        {copyableElement}
      </Trigger>
    ) : (
      copyableElement
