      ...
      <div css={listAvatarAndNameContainerStyle}>
        ...
        <div css={nickNameAndEmailContainerStyle}>
          <div css={nickNameContainerStyle}>
            <span>{nickName}</span>
            {status === USER_STATUS.PENDING && <span> ({t('user_management.status.pending')})</span>}
            {userID === currentUserID && <span css={statusStyle}> ({t('user_management.status.current-user')})</span>}
          </div>
