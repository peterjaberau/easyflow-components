...
import { ChangeEvent, FC, useRef, useState, MouseEvent, useEffect, forwardRef } from 'react';
import React from 'react';
import { GetHeaderOperationsFun, DatePickerModeType, SingleDatePickerProps } from './interface';
...
export const SingleDatePicker = forwardRef<HTMLDivElement, SingleDatePickerProps>((props, ref) => {
  const {
    allowClear = true,
    placeholder,
    disabled,
  ...
  const [value, setValue] = useState<Dayjs | undefined>(
    getDefaultValue(format, propsValue, defaultValue, utcOffset, timezone) as Dayjs,
  );
  const [popupVisible, setPopupVisible] = useState<boolean | undefined>(!!props.popupVisible);
  const mergedPopupVisible = 'popupVisible' in props ? props.popupVisible : popupVisible;
  const mergedValue =
    'value' in props ? (getDayjsValue(propsValue as Dayjs, format, utcOffset, timezone) as Dayjs) : value;

  const defaultPageShowDate = mergedValue || (getDayjsValue(defaultPickerValue as Dayjs, format) as Dayjs) || getNow();

  const [inputValue, setInputValue] = useState<string | undefined>();
  const [valueShow, setValueShow] = useState<Dayjs>();
  const [pageShowDate, setPageShowDate] = useState<Dayjs | undefined>(defaultPageShowDate);
  const mergedPageShowDate = (getDayjsValue(pickerValue as Dayjs, format) as Dayjs) || pageShowDate;
  ...
  const defaultTimeValue =
    (isObject(showTime) &&
      (getDayjsValue(showTime.defaultValue as Dayjs, showTime.format || 'HH:mm:ss', utcOffset, timezone) as Dayjs)) ||
  ...
  function handlePickerValueChange(v?: Dayjs) {
    const finalValue = typeof format === 'function' ? format(v as Dayjs) : v?.format(format);
  ...
  function onClickConfirmBtn() {
    ...
    onConfirmValue();
    const finalValue = typeof format === 'function' ? format(pv as Dayjs) : pv?.format(format);
  ...
  function onHandleSelect(_: string | undefined, date?: Dayjs, now?: boolean) {
    ...
    if (showTime) {
      ...
      const localTime = getLocaleDayjsValue(toLocal(newTime as Dayjs, utcOffset, timezone), 'en-us');
      const finalValue = typeof format === 'function' ? format(localTime as Dayjs) : localTime?.format(format);

      onSelect && onSelect(finalValue, localTime);
    } else {
      const localTime = getLocaleDayjsValue(toLocal(date as Dayjs, utcOffset, timezone).locale('en-us'), 'en-us');
      const finalValue = typeof format === 'function' ? format(localTime as Dayjs) : localTime?.format(format);
      onSelect && onSelect(localTime ? localTime.format(finalValue) : undefined, localTime as Dayjs);
      setValue(date);
      onHandleChange(date);
      setOpen(false);
    }
  ...
  function onHandleChange(newValue: Dayjs | undefined) {
    if (isDayjsChange(newValue, mergedValue)) {
      const finalValue = typeof format === 'function' ? format(newValue as Dayjs) : newValue?.format(format);
  ...
  function onTimePickerSelect(_: string, time: Dayjs) {
    ...
    const localNewValueShow = getLocaleDayjsValue(toLocal(newValueShow, utcOffset, timezone), 'en-us');
    const finalValue =
      typeof format === 'function' ? format(localNewValueShow as Dayjs) : localNewValueShow?.format(format);
  ...
  function isValid(time: string | Dayjs): boolean {
    return (
      ...
      (typeof disabledDate === 'function'
        ? !disabledDate(typeof format === 'function' ? dayjsPro(format(dayjsPro(time))) : dayjsPro(time, format))
        : true)
    );
  }
  ...
  function onChangeInput(e: ChangeEvent<HTMLInputElement>) {
    ...
    if (isValid(niv)) {
      const newValue = getDayjsValue(niv, format as string, utcOffset, timezone) as Dayjs;
  ...
  function onSelectNow() {
    ...
    const finalValue = typeof format === 'function' ? format(now as Dayjs) : now?.format(format);
    onHandleSelect(finalValue, now, true);
  }

  const suffixIcon = inputSuffix === null ? null : inputSuffix || <CalendarIcon />;

  const shouldShowFooter =
    (showTime && panelMode === 'date') || extra || (!showTime && panelMode === 'date' && showNowBtn);
