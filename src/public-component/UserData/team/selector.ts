...
export const getCurrentMemberList = (state: RootState) => state.team.currentMemberList;

export const getCurrentTeamInfo: any = createSelector([getCurrentId, getTeamItems], (currentId, items) => {
  if (!currentId || !items) return;
  return items.find((item) => item.id === currentId);
});

export const getCurrentTeamTotalLicense: any = createSelector([getCurrentTeamInfo], (teamInfo) => {
  if (!teamInfo) return;
  return teamInfo.totalTeamLicense;
});

export const getUserRoleInCurrentTeam: any = createSelector([getCurrentTeamInfo], (teamInfo) => {
  if (!teamInfo) return USER_ROLE.VIEWER;
  return teamInfo.myRole;
});

export const getCurrentTeamIdentifier: any = createSelector([getCurrentId, getTeamItems], (currentId, items) => {
  if (!currentId || !items) return;
...
export const getCurrentTeamPlan: any = createSelector([getCurrentTeamInfo], (currentTeamInfo) => {
