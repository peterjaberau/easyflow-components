...
import { isArray, useMergeValue } from '../system';
import { CheckboxGroupProps, CheckboxOption, CheckboxProps } from './interface';
import { SerializedStyles } from '@emotion/react';
import { Checkbox } from './checkbox';
import { applyCheckboxContainerHorizontal, applyCheckboxContainerVertical } from './style';
...
export const CheckboxGroup = forwardRef<HTMLDivElement, CheckboxGroupProps>((props, ref) => {
  ...
  return (
    <div css={[checkboxGroupCss, applyBoxStyle(props)]} ref={ref} {...deleteCssProps(otherProps)}>
      <CheckboxGroupContext.Provider value={props}>
        {isArray(options)
                  ...
                  <Checkbox
                    ...
                    onChange={(checked, event) => {
                      ...
                      if (checked) {
                        ...
                        finalValue = currentValue.filter((value) => value !== optionObject.value);
                      }
                      if (value === undefined) {
          ...
          : Children.map(children, (child, index) => {
              const item = child as ReactElement<PropsWithChildren<CheckboxProps>>;
              return cloneElement(item, {
                ...
                onChange: (checked, event) => {
                  ...
                  if (checked) {
                    ...
                    finalValue = currentValue.filter((value) => value !== item.props.value);
                  }
                  if (value === undefined) {
                    setCurrentValue(finalValue as []);
                  }
