...
export const RadioGroup = forwardRef<HTMLDivElement, RadioGroupProps<any>>((props, ref) => {
  ...
  return (
    <div
      ref={ref}
      css={[applyRadioGroupCss({ hasChildren, direction, spacing, type, size }), applyBoxStyle(props)]}
        ...
        {options && isArray(options)
          ? options.map((option, index) => {
              return typeof option === 'string' || typeof option === 'number' ? (
                ...
                <Radio key={`radio-${index}`} value={option.value} disabled={disabled || option.disabled}>
                  {option.label}
                </Radio>
              );
            })
