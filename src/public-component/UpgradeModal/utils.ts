...
import { SUBSCRIBE_PLAN, TeamInfo } from '../publicTypes';
import { getILLACloudURL, isIllaErrorInterface } from '../utils';
import { createCollarModal, createTeamLimitModal } from './hook';
import { CollarModalType, FREE_TEAM_LIMIT_TYPE } from './interface';

export function getSuccessRedirectWithParams(params: Record<string, string | number>): string {
...
export const handleCollaPurchaseError = (e: unknown, modalType: CollarModalType, from: string) => {
  ...
  if (
    ...
    (e.data.errorFlag === ERROR_FLAG.ERROR_FLAG_INSUFFICIENT_COLLA ||
      ...
      e.data.errorFlag === ERROR_FLAG.ERROR_FLAG_INSUFFICIENT_AI_TOKEN_GENERAL ||
      ...
      e.data.errorFlag === ERROR_FLAG.ERROR_FLAG_AI_AGENT_MAX_TOKEN_OVER_COLLA_BALANCE)
  ) {
    collaModal?.({
      modalType,
      from,
...
export const handleCollaPurchaseErrorByILLAInnerError = (e: unknown, modalType: CollarModalType, from: string) => {
  const collaModal = createCollarModal();
  if (
    ...
    (e.errorFlag === ERROR_FLAG.ERROR_FLAG_INSUFFICIENT_COLLA ||
      ...
      e.errorFlag === ERROR_FLAG.ERROR_FLAG_AI_AGENT_MAX_TOKEN_OVER_COLLA_BALANCE)
  ) {
    collaModal?.({
      modalType,
      from,
...
export const handleFreeTeamLimitError = (e: unknown, modalType: FREE_TEAM_LIMIT_TYPE) => {
  const limitTeamModal = createTeamLimitModal();
  if (isILLAAPiError(e) && e.data.errorFlag === ERROR_FLAG.ERROR_FLAG_OVER_MAX_FREE_TEAM_LIMIT) {
    limitTeamModal?.({
      modalType,
    });
    return true;
...
export const isSubscribeForUseDrive = (teamInfo: TeamInfo) => {
  ...
  const subscribeCollaPlans = [SUBSCRIBE_PLAN.COLLA_SUBSCRIBE_PAID, SUBSCRIBE_PLAN.COLLA_SUBSCRIBE_CANCELED];
