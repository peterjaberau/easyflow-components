...
import { DEFAULT_WEEK_START, getAllDaysByTime, getTimeFormat } from '../../utils/dateHelper';
import { datePanelStyle, timePickerBodyPanelStyle, timePickerHeaderPanelStyle } from './style';
import { weekPanelStyle } from '../week/style';
import { BasicHeaderSection } from '../basic-header-section';
import { NextDoubleIcon, NextIcon, PreviousDoubleIcon, PreviousIcon } from '../../../icon';
import { YearPickerPanel } from '../year';
import { MonthPickerPanel } from '../month';
import { dayjsPro, getDayjsValue, isObject } from '../../../system';
...
const defaultIsSameTime = (current: Dayjs, target: Dayjs) => current.isSame(target, 'day');
...
export const DatePickerPanel: FC<DatePickerPanelProps> = (props) => {
  ...
  const DATE_PICKER_LOCALE = configProviderProps?.locale?.datePicker ?? def.datePicker;

  const timeFormat = (isObject(showTime) && showTime.format) || getTimeFormat(format as string);

  const innerIsSameTime = useMemo(() => isSameTime || defaultIsSameTime, [isSameTime]);
  ...
  const disabledTimeProps = useMemo(() => {
    if (isRangePicker) {
      return typeof disabledTime === 'function' ? disabledTime(rangeValues?.[index ?? 0] as Dayjs) : {};
    } else {
      return typeof disabledTime === 'function'
        ? disabledTime?.(getDayjsValue(value as Dayjs, format as string) as Dayjs)
        : {};
    }
    ...
    <div style={{ display: 'flex' }}>
      ...
      {showTime && (
        <div css={timePickerBodyPanelStyle}>
          <div css={timePickerHeaderPanelStyle}>{timeValue?.format('HH:mm:ss') ?? DATE_PICKER_LOCALE.timeText}</div>
          <TimePickerPopup
            {...timePickerProps}
            {...disabledTimeProps}
            hideFooter
