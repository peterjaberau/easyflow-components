...
import { FilterOperator, FilterOptionsState, TableContextProps, TableProps } from './interface';
import {
  ReactElement,
  SyntheticEvent,
  useCallback,
...
import { customGlobalFn, downloadDataAsCSV, transformRawDataIntoCsvData, transformTableIntoCsvData } from './utils';
import { isNumber, isString } from '../system';
import {
...
import { Tr } from './tr';
import { Th } from './th';
import { DownloadIcon, SorterDefaultIcon, SorterDownIcon, SorterUpIcon, RefreshIcon } from '../icon';
...
export function RenderDataDrivenTable<D extends TableData, TValue>(props: TableProps<D, TValue>): ReactElement {
  const {
    tableRef,
    size = 'medium',
    tableLayout = 'auto',
  ...
  const [filterOption, setFilterOption] = useState<FilterOptionsState>(DEFAULT_TABLE_FILTER);
  ...
  const [columnSizing, setColumnSizing] = useState<ColumnSizingState>(_columnSizing);
  ...
  return (
    <div
      ref={containerRef}
      css={[applyContainerStyle(), applyBoxStyle(props), applyBorderedStyle(bordered)]}
      style={{ padding: '' }}
    >
      <Spin loading={loading} pos="static" colorScheme={colorScheme} css={spinStyle}>
        <TableContext.Provider value={contextProps}>
          <table css={applyTableStyle(tableLayout)} {...deleteCssProps(otherProps)}>
            {showHeader && (
              <Thead pinedHeader={pinedHeader}>
                {table.getHeaderGroups().map((headerGroup, index) => (
                  <Tr key={headerGroup.id} hoverable>
                    {headerGroup.headers.map((header) => {
                      const lastCol = headerGroup.headers.indexOf(header) === headerGroup.headers.length - 1;
                      ...
                      return (
                        <Th
                          ...
                          lastCol={lastCol}
                          lastRow={table.getHeaderGroups().indexOf(headerGroup) === table.getHeaderGroups().length - 1}
                          customCellPadding={customCellPadding}
                        >
                          <div css={applyPreContainer(align)} onClick={() => header.column.toggleSorting()}>
              ...
              {table.getRowModel().rows.map((row, index) => (
                ...
                >
                  {row.getVisibleCells().map((cell) => {
                    const bgColor = cell.column.columnDef.meta?.getBackgroundColor?.(cell.getContext());
                    return (
                      <Td
                        ...
                        lastCol={row.getVisibleCells().indexOf(cell) === row.getVisibleCells().length - 1}
                        lastRow={table.getRowModel().rows.indexOf(row) === table.getRowModel().rows.length - 1}
                        customCellPadding={customCellPadding}
                        css={[cell.column.columnDef?.meta?.style, applyTableCellBackgroundStyle(bgColor)]}
                        selected={selectedCell === cell.id}
                        onClick={(e) => {
                          if (multiRowSelection && cell.column.id === MULTI_ROW_SELECTION_CHECKBOX_ID) {
              ...
              {table.getRowModel().rows.length ? null : (
                <tr>
                  <td colSpan={99} style={{ textAlign: 'center' }}>
                    <Empty opac={serverSidePagination && loading ? 0 : undefined} {...emptyProps} />
                  </td>
                </tr>
              )}
            </TBody>
                  ...
                  <Tr key={footerGroup.id} hoverable>
                    {footerGroup.headers.map((header) => (
                      <Th
                        ...
                        lastCol={footerGroup.headers.indexOf(header) === footerGroup.headers.length - 1}
                        lastRow={table.getHeaderGroups().indexOf(footerGroup) === table.getHeaderGroups().length - 1}
                        customCellPadding={customCellPadding}
                      >
      ...
      {overFlow === 'pagination' || download || filter ? (
        <div css={toolBarStyle}>
          ...
          {overFlow === 'pagination' ? (
            <Pagination
              {...pagination}
              total={serverSidePagination && isNumber(total) ? total : Object.keys(table.getRowModel().rowsById).length}
              pageSize={pageSize}
              current={pageIndex + 1}
