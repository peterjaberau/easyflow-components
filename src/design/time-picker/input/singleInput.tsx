...
import { forwardRef, useImperativeHandle, useMemo, useRef, KeyboardEvent, useCallback } from 'react';
...
export const DateInput = forwardRef<DateInputHandler, DateInputProps>((props, ref) => {
  const {
    style,
    className,
  ...
  const showValue = useMemo(() => {
    if (inputValue !== undefined) {
      ...
      return typeof format === 'function' ? format(value) : value.format(format);
    }
    return '';
  ...
  return (
    <div
      ...
      css={applyInputWrapperStyle(size, colorScheme, disabled, error, allowClear && !!showValue)}
    >
