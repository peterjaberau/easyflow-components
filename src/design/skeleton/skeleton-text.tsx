...
export function SkeletonText(props: SkeletonTextProps) {
  ...
  const lines = Array.from({ length: rows }, (_, idx) => {
    ...
    return <li key={idx} css={[applyLineStyle(lineWidth), applyAnimation(animation)]} />;
  });

  return (
    <ul css={[textContainerStyle, applyBoxStyle(props)]} {...deleteCssProps(restProps)}>
      {lines}
    </ul>
  );
}
