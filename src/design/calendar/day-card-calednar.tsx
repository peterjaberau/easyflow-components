...
import { ConfigProviderContext, ConfigProviderProps, def, koKR, zhCN } from '../config-provider';
...
export const DayCardCalendar = forwardRef<HTMLDivElement, CardCalendarProps>((props, ref) => {
  const { colorScheme, value, currentDate, onChange, onCurrentDateChange, panelTodayBtn, ...otherProps } = props;
    ...
    monthDay.push(
      <div
        ...
        css={applyMonthBlockDayStyle(
          ...
          currentDay.year() === currentDate.year() && currentDay.month() === currentDate.month(),
  ...
  const title = useMemo(() => {
    if (configProviderProps?.locale === zhCN || configProviderProps?.locale === koKR) {
      return `${currentDate.year()}${locale['year']}${currentDate.month() + 1}${locale['month']}`;
    } else {
      return `${getMonthString(currentDate.month(), locale)} ${currentDate.year()}`;
    }
  }, [configProviderProps?.locale, currentDate, locale]);

  return (
    <div css={[applyDayCardCalendarStyle(), applyBoxStyle(otherProps)]} ref={ref} {...deleteCssProps(otherProps)}>
      ...
      <div css={applyDayCardBlockStyle()}>
        <div css={applyMonthBlockHeaderContainerStyle()}>
          ...
          <span css={applyMonthBlockHeaderStyle()}>{locale['Wednesday']}</span>
