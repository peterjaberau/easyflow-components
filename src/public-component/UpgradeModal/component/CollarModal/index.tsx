...
import { SUBSCRIPTION_CYCLE, USER_ROLE } from '../../../publicTypes';
import { getCurrentTeamInfo, getCurrentUserID } from '../../../UserData';
import { FC, useEffect } from 'react';
import { useTranslation } from 'react-i18next';
import { useSelector } from 'react-redux';
import { Button, CloseIcon, Modal } from '../../../../design/react';
...
export const CollarModal: FC<CollarModalProps> = (props) => {
  const { visible, from, modalType = CollarModalType.TOKEN, onCancel, afterClose } = props;
  ...
  const reportElement = isSubscribe ? 'colla_increase_modal' : 'colla_subscribe_modal';
  ...
  return (
    ...
    >
      ...
      <div>
        <div css={footerStyle}>
          <div>
            <div css={priceStyle}>{`$${COLLAR_UNIT_PRICE[SUBSCRIPTION_CYCLE.MONTHLY]}`}</div>
            <div css={priceContentStyle}>{t('billing.modal.colla_insufficient_modal.cycle')}</div>
          </div>
          <Button css={upgradeButtonStyle} leftIcon={<UpgradeIcon />} colorScheme="techPurple" onClick={handleClick}>
