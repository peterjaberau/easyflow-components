...
import { globalColor, illaPrefix, zIndex } from '../theme';
...
export function applyNotificationSlide(position: NotificationPosition): Variants {
  const originX = position === 'topLeft' || position === 'bottomLeft' ? '-100%' : '100%';
  return {
    initial: {
      x: `${originX}`,
    },
...
export function applyNotificationContainerStyle(position: NotificationPosition): SerializedStyles {
