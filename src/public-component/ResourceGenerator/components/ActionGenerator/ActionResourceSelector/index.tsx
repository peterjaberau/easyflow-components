...
import { AddIcon, Button, ButtonGroup, List, PreviousIcon } from '../../../../../design/react';
...
export const ActionResourceSelector: FC<ActionResourceSelectorProps> = (props) => {
  const { actionType, canBack, onBack, onCreateAction, onCreateResource, handleCreateAction } = props;
  ...
  const [selectedResourceId, setSelectedResourceId] = useState<string>(resourceList[0]?.resourceID);
  ...
  }, [actionType, handleCreateAction, onCreateAction, selectedResourceId, track]);
      ...
      <List
        ...
        render={(r) => {
          return (
            ...
            >
              <Suspense fallback={<div>Loading...</div>}>{getIconFromResourceType(r.resourceType, '24px')}</Suspense>
              <span css={resourceItemTitleStyle}>{r.resourceName}</span>
              <span css={resourceItemTimeStyle}>{t('created_at') + ' ' + fromNow(r.createdAt)}</span>
            </div>
          );
        }}
      />
