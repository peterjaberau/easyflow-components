...
import { ILLA_MIXPANEL_EVENT_TYPE } from '../../../MixpanelUtils';
import { PROMOTION_CODE_USAGE, SUBSCRIBE_PLAN, SUBSCRIPTION_CYCLE, USER_ROLE } from '../../../publicTypes';
import { TextLink } from '../../../TextLink';
import { getCurrentId, getCurrentTeamInfo, getCurrentUser, getCurrentUserID } from '../../../UserData';
import { isMobileByWindowSize } from '../../../utils';
import { FC, useEffect, useMemo, useState } from 'react';
import { Trans, useTranslation } from 'react-i18next';
import { useSelector } from 'react-redux';
import { useWindowSize } from 'react-use';
import { Button, CloseIcon, Divider, Drawer, InputNumber, Link, Radio, useMessage } from '../../../../design/react';
import { PURCHASE_TYPE } from '../../interface';
import { cancelSubscribe, modifySubscribe, subscribe } from '../../service';
import { LICENSE_NEW_USER_DISCOUNT, LICENSE_UNIT_PRICE } from '../../service/interface';
import { getSuccessRedirectWithParams, isSubscribeForDrawer, track } from '../../utils';
...
export const UpgradeDrawer: FC<UpgradeDrawerProps> = (props) => {
  ...
  const canUseDiscount = useMemo(() => {
    ...
    const index = promotionCodes.findIndex((item) => {
      ...
      return item.usage === PROMOTION_CODE_USAGE.DEFAULT_REGISTER && item.maxRedemptions > 0 && ddl - now > 0;
    });
    return index !== -1;
  }, [promotionCodes]);
  ...
  const [quantity, setQuantity] = useState<number>(defaultConfig?.subscribeInfo?.quantity || 1);
  ...
  const monthUnitePrice = cycle === SUBSCRIPTION_CYCLE.MONTHLY ? `$${unitPrice}` : `$${(unitPrice / 12).toFixed(1)}`;
  ...
  const actionDisabled = useMemo(() => {
    ...
    if (!isSubscribeForDrawer(subscribeInfo?.plan) || subscribeInfo?.cancelAtPeriodEnd) {
      return quantity === 0;
    }
    return subscribeInfo?.quantity === quantity && subscribeInfo?.cycle === cycle;
  ...
  const actionButtonText = useMemo(() => {
    ...
    const statusLabelKeys = {
      ...
      subscribed_plan_decreased_with_update: 'billing.payment_sidebar.button.change_plan',
      subscribed_plan_increased_with_update: 'billing.payment_sidebar.button.change_plan',
  ...
  const handleSubscribe = async () => {
    ...
    const successRedirect = getSuccessRedirectWithParams({
      ...
      purchaseValue: canUseDiscount ? unitPrice * quantity * LICENSE_NEW_USER_DISCOUNT : unitPrice * quantity,
    });
  ...
  }, [currentTeamInfo?.id, currentTeamInfo?.myRole, from, reportElement, userID, visible]);
      ...
      <div css={drawerContentStyle}>
        <div>
          <div css={drawerPaddingStyle}>
            <CloseIcon containerStyle={closeIconStyle} onClick={handleOnClose} />
            <div css={titleStyle}>{t('billing.payment_sidebar.title.manage_licenses')}</div>
            <div css={manageItemStyle}>
              <div css={radioContainerStyle}>
                ...
                >
                  <span css={labelStyle}>{t('billing.payment_sidebar.select_option.Monthly')}</span>
                </Radio>
                <div css={hasExtraRadioStyle}>
                  ...
                  >
                    <span css={labelStyle}>{t('billing.payment_sidebar.select_option.Yearly')}</span>
                  </Radio>
                  <span css={extraStyle}>{t('billing.new_pricing.buy_10_months')}</span>
                </div>
              </div>
              <div>
                <span css={monthPriceStyle}>{monthUnitePrice}</span>
                <span css={monthUnitStyle}>{t('billing.first_time_off.unit')}</span>
              </div>
              <InputNumber
                mode="button"
                ...
                min={isSubscribeForDrawer(defaultConfig?.subscribeInfo?.plan) ? 0 : 1}
              />
            </div>
          </div>
          <Divider />
          <div css={drawerPaddingStyle}>
            <div css={subTotalStyle}>
              <span css={subTotalLeftStyle}>{t('billing.payment_sidebar.price_label.total')}</span>
              <div css={priceStyle}>
                <div css={priceLabelContainerStyle}>
                  <span css={priceTotalLabelStyle}>{priceLabel}</span>
                  {canUseDiscount && <span css={discountStyle}>{t('billing.first_time_off.discount')}</span>}
                </div>
                <span css={priceTotalStyle}>
                  ${(unitPrice * quantity * (canUseDiscount ? LICENSE_NEW_USER_DISCOUNT : 1)).toFixed(2)}
                </span>
              </div>
            </div>
            <Button
        ...
        <div css={drawerPaddingStyle}>
          {defaultConfig?.appSumoInvoiceURL ? (
            <div css={textCenterStyle}>
              <Link _css={appSumoLinkStyle} colorScheme="techPurple" href={defaultConfig?.appSumoInvoiceURL}>
